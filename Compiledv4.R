...
firstqpoints <- lapply(next_team_batch$idPlayer, function(x){
  ...
  output_season <- lapply(season_types, function(y){
    ...
    firstq_makes <- playerdata %>% filter(idPlayer == x, typeSeason == y, slugSeason == "2024-25") %>% 
    ...
    mutate(pts = replace_na(pts,0))
      ...
      firstq_makes %>% ungroup() %>% mutate(test = mean(pts > x), OU = x) %>% group_by(namePlayer, idPlayer, typeSeason, OU) %>% 
...
firstqassists <- lapply(next_team_batch$idPlayer, function(x){
  ...
  firstq_assists <- playerdata %>% filter(idPlayer == x, typeSeason == y, slugSeason == "2024-25") %>% 
  ...
  hit_rate_above <- lapply(hit_rate, function(x){
    ...
    firstq_assists %>% ungroup() %>% mutate(test = mean(assists > x), OU = x) %>% group_by(namePlayer, idPlayer, typeSeason, OU) %>% 
      summarize(test = min(test), .groups = 'drop') %>% 
      ungroup() %>% mutate(slugTeam = slug_team)
...
firstqrebounds <- lapply(next_team_batch$idPlayer, function(x){
  ...
  play_play_rebounds <- play_play_player_rebounds %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,slugTeamPlayer1) %>% 
    rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam,slugSeason) %>% 
  ...
  firstq_rebounds <- playerdata %>% filter(idPlayer == x, typeSeason == y, slugSeason == "2024-25")  %>% 
    group_by(dateGame,locationGame,typeSeason,slugOpponent,slugSeason,namePlayer,idPlayer,slugTeam) %>% summarize(n = n()) %>% 
  ...
  hit_rate_above <- lapply(hit_rate, function(x){
    ...
    firstq_rebounds %>% ungroup() %>% mutate(test = mean(rebounds > x), OU = x) %>% group_by(namePlayer, idPlayer, typeSeason, OU) %>% summarize(test = min(test), .groups = 'drop') %>% 
      ungroup() %>% mutate(slugTeam = slug_team)
...
rmarkdown::render(input = 'C:/Users/CECRAIG/Desktop/Backironanalytics/my-site-test/Matrix_Testv3.Rmd',
