...
test_playoffs <- parLapply(cl,test_names,test_playoffs)
...
test_home_playoffs <- parLapply(cl,test_names,test_home_playoffs)


#Test Away


test_away_playoffs <- function(z){
...
test_away_playoffs <- parLapply(cl,test_names,test_away_playoffs)


#Test Last 5


test_five_playoffs <- function(z){
...
test_five_playoffs <- parLapply(cl,test_names,test_five_playoffs)


#Test Last 10


test_ten_playoffs <- function(z){
...
test_ten_playoffs <- parLapply(cl,test_names,test_ten_playoffs)
...
test_playoffs <- bind_rows(test_playoffs)
test_2_playoffs <- test_playoffs %>% pivot_wider(names_from = OU, values_from = test)%>% unnest(cols = everything())

fg3a_playoffs <- test_2_playoffs %>% filter(metric == "fg3a") %>% select(!metric)
ftm_playoffs <- test_2_playoffs %>% filter(metric == "ftm") %>% select(!metric)
fgm_playoffs <- test_2_playoffs %>% filter(metric == "fgm") %>% select(!metric)
ptrebast_1_playoffs <- test_2_playoffs %>% filter(metric == "pts_reb_ast") %>% select(!metric)
pt_reb_1_playoffs <- test_2_playoffs %>% filter(metric == "pts_reb") %>% select(!metric)
ast_reb_1_playoffs <- test_2_playoffs %>% filter(metric == "ast_reb") %>% select(!metric)
pt_ast_1_playoffs <- test_2_playoffs %>% filter(metric == "pts_ast") %>% select(!metric)
stl_blk_1_playoffs <- test_2_playoffs %>% filter(metric == "stl_blk") %>% select(!metric)
fg3m_1_playoffs <- test_2_playoffs %>% filter(metric == "fg3m") %>% select(!metric)
stl_1_playoffs <- test_2_playoffs %>% filter(metric == "stl") %>% select(!metric)
blk_1_playoffs <- test_2_playoffs %>% filter(metric == "blk") %>% select(!metric)
tov_1_playoffs <- test_2_playoffs %>% filter(metric == "tov") %>% select(!metric)
pt_playoffs <- test_2_playoffs %>% filter(metric == "pts") %>% select(!metric)
ast_1_playoffs <- test_2_playoffs %>% filter(metric == "ast") %>% select(!metric)
reb_playoffs <- test_2_playoffs %>% filter(metric == "treb") %>% select(!metric)

fg3a_pivoted_playoffs <- fg3a_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                     summarize(GP = n()), by = "idPlayer")   %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam)  %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 

ftm_pivoted_playoffs <- ftm_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                   summarize(GP = n()), by = "idPlayer")  %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam) %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 

fgm_pivoted_playoffs <- fgm_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                   summarize(GP = n()), by = "idPlayer")  %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam) %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 

ptrebast_pivoted_playoffs <- ptrebast_1_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                               summarize(GP = n()), by = "idPlayer")  %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam)   %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 

ptrebast_playoffs <- test_playoffs %>% filter(metric == "pts_reb_ast") %>% mutate(Type = "Playoffs")

pt_reb_pivoted_playoffs <- pt_reb_1_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                           summarize(GP = n()), by = "idPlayer")  %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam)  %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 

pt_reb_playoffs <- test_playoffs %>% filter(metric == "pts_reb") %>% mutate(Type = "Playoffs")

ast_reb_pivoted_playoffs <- ast_reb_1_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                             summarize(GP = n()), by = "idPlayer")  %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam)  %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 

ast_reb_playoffs <- test_playoffs %>% filter(metric == "ast_reb") %>% mutate(Type = "Playoffs")


pt_ast_pivoted_playoffs <- pt_ast_1_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                           summarize(GP = n()), by = "idPlayer")  %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam)  %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 

pt_ast_playoffs <- test_playoffs %>% filter(metric == "pts_ast") %>% mutate(Type = "Playoffs")


stl_blk_pivoted_playoffs <- stl_blk_1_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                             summarize(GP = n()), by = "idPlayer")  %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam)   %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 

stl_blk_playoffs <- test_playoffs %>% filter(metric == "stl_blk") %>% mutate(Type = "Playoffs")

fg3m_pivoted_playoffs <- fg3m_1_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                       summarize(GP = n()), by = "idPlayer")  %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam) %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam)

fg3m_playoffs <- test_playoffs %>% filter(metric == "fg3m")%>% mutate(Type = "Playoffs")


stl_pivoted_playoffs <- stl_1_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                     summarize(GP = n()), by = "idPlayer")   %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam)  %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 

stl_playoffs <- test_playoffs %>% filter(metric == "stl") %>% mutate(Type = "Playoffs")


blk_pivoted_playoffs <- blk_1_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                     summarize(GP = n()), by = "idPlayer")  %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam) %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 

blk_playoffs <- test_playoffs %>% filter(metric == "blk") %>% mutate(Type = "Playoffs")

tov_pivoted_playoffs <- tov_1_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                     summarize(GP = n()), by = "idPlayer")  %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam) %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 

tov_playoffs <- test_playoffs %>% filter(metric == "tov") %>% mutate(Type = "Playoffs")


pt_pivoted_playoffs <- pt_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                 summarize(GP = n()), by = "idPlayer")  %>% 
  left_join(teams, by = "slugTeam")   %>% rename(Player = namePlayer, Team = slugTeam)  %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 

pts_playoffs <- test_playoffs %>% filter(metric == "pts") %>% mutate(Type = "Playoffs")

ast_pivoted_playoffs <- ast_1_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                     summarize(GP = n()), by = "idPlayer")   %>% 
  left_join(teams, by = "slugTeam")   %>% rename(Player = namePlayer, Team = slugTeam)   %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 

ast_playoffs <- test_playoffs %>% filter(metric == "ast") %>% mutate(Type = "Playoffs")


reb_pivoted_playoffs <- reb_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                   summarize(GP = n()), by = "idPlayer")   %>% 
  left_join(teams, by = "slugTeam")   %>% rename(Player = namePlayer, Team = slugTeam) %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 

treb_playoffs <- test_playoffs %>% filter(metric == "treb") %>% mutate(Type = "Playoffs")



test_home_playoffs <- bind_rows(test_home_playoffs)
test_2_home_playoffs <- test_home_playoffs %>% pivot_wider(names_from = OU, values_from = test)%>% unnest(cols = everything())

fg3a_home_playoffs <- test_home_playoffs %>% filter(metric == "fg3a") %>% select(!metric) %>% mutate(Type = "Home Games")
ftm_home_playoffs <- test_home_playoffs %>% filter(metric == "ftm") %>% select(!metric) %>% mutate(Type = "Home Games")
fgm_home_playoffs <- test_home_playoffs %>% filter(metric == "fgm") %>% select(!metric) %>% mutate(Type = "Home Games")
ptrebast_home_playoffs <- test_home_playoffs %>% filter(metric == "pts_reb_ast") %>% select(!metric) %>% mutate(Type = "Home Games")
pt_reb_home_playoffs <- test_home_playoffs %>% filter(metric == "pts_reb") %>% select(!metric) %>% mutate(Type = "Home Games")
ast_reb_home_playoffs <- test_home_playoffs %>% filter(metric == "ast_reb") %>% select(!metric) %>% mutate(Type = "Home Games")
pt_ast_home_playoffs <- test_home_playoffs %>% filter(metric == "pts_ast") %>% select(!metric) %>% mutate(Type = "Home Games")
stl_blk_home_playoffs <- test_home_playoffs %>% filter(metric == "stl_blk") %>% select(!metric) %>% mutate(Type = "Home Games")
fg3m_home_playoffs <- test_home_playoffs %>% filter(metric == "fg3m") %>% select(!metric) %>% mutate(Type = "Home Games")
stl_home_playoffs <- test_home_playoffs %>% filter(metric == "stl") %>% select(!metric) %>% mutate(Type = "Home Games")
blk_home_playoffs <- test_home_playoffs %>% filter(metric == "blk") %>% select(!metric) %>% mutate(Type = "Home Games")
tov_home_playoffs <- test_home_playoffs %>% filter(metric == "tov") %>% select(!metric) %>% mutate(Type = "Home Games")
pts_home_playoffs <- test_home_playoffs %>% filter(metric == "pts") %>% select(!metric) %>% mutate(Type = "Home Games")
ast_home_playoffs <- test_home_playoffs %>% filter(metric == "ast") %>% select(!metric) %>% mutate(Type = "Home Games")
treb_home_playoffs <- test_home_playoffs %>% filter(metric == "treb") %>% select(!metric) %>% mutate(Type = "Home Games")



test_away_playoffs <- bind_rows(test_away_playoffs)
test_2_away_playoffs <- test_away_playoffs %>% pivot_wider(names_from = OU, values_from = test)%>% unnest(cols = everything())

fg3a_away_playoffs <- test_away_playoffs %>% filter(metric == "fg3a") %>% select(!metric) %>% mutate(Type = "Away Games")
ftm_away_playoffs <- test_away_playoffs %>% filter(metric == "ftm") %>% select(!metric) %>% mutate(Type = "Away Games")
fgm_away_playoffs <- test_away_playoffs %>% filter(metric == "fgm") %>% select(!metric) %>% mutate(Type = "Away Games")
ptrebast_away_playoffs <- test_away_playoffs %>% filter(metric == "pts_reb_ast") %>% select(!metric) %>% mutate(Type = "Away Games")
pt_reb_away_playoffs <- test_away_playoffs %>% filter(metric == "pts_reb") %>% select(!metric) %>% mutate(Type = "Away Games")
ast_reb_away_playoffs <- test_away_playoffs %>% filter(metric == "ast_reb") %>% select(!metric) %>% mutate(Type = "Away Games")
pt_ast_away_playoffs <- test_away_playoffs %>% filter(metric == "pts_ast") %>% select(!metric) %>% mutate(Type = "Away Games")
stl_blk_away_playoffs <- test_away_playoffs %>% filter(metric == "stl_blk") %>% select(!metric) %>% mutate(Type = "Away Games")
fg3m_away_playoffs <- test_away_playoffs %>% filter(metric == "fg3m") %>% select(!metric) %>% mutate(Type = "Away Games")
stl_away_playoffs <- test_away_playoffs %>% filter(metric == "stl") %>% select(!metric) %>% mutate(Type = "Away Games")
blk_away_playoffs <- test_away_playoffs %>% filter(metric == "blk") %>% select(!metric) %>% mutate(Type = "Away Games")
tov_away_playoffs <- test_away_playoffs %>% filter(metric == "tov") %>% select(!metric) %>% mutate(Type = "Away Games")
pts_away_playoffs <- test_away_playoffs %>% filter(metric == "pts") %>% select(!metric) %>% mutate(Type = "Away Games")
ast_away_playoffs <- test_away_playoffs %>% filter(metric == "ast") %>% select(!metric) %>% mutate(Type = "Away Games")
treb_away_playoffs <- test_away_playoffs %>% filter(metric == "treb") %>% select(!metric) %>% mutate(Type = "Away Games")




test_five_playoffs <- bind_rows(test_five_playoffs)
test_2_five_playoffs <- test_five_playoffs %>% pivot_wider(names_from = OU, values_from = test)%>% unnest(cols = everything())

fg3a_five_playoffs <- test_five_playoffs %>% filter(metric == "fg3a") %>% select(!metric) %>% mutate(Type = "Last 5")
ftm_five_playoffs <- test_five_playoffs %>% filter(metric == "ftm") %>% select(!metric) %>% mutate(Type = "Last 5")
fgm_five_playoffs <- test_five_playoffs %>% filter(metric == "fgm") %>% select(!metric) %>% mutate(Type = "Last 5")
ptrebast_five_playoffs <- test_five_playoffs %>% filter(metric == "pts_reb_ast") %>% select(!metric) %>% mutate(Type = "Last 5")
pt_reb_five_playoffs <- test_five_playoffs %>% filter(metric == "pts_reb") %>% select(!metric) %>% mutate(Type = "Last 5")
ast_reb_five_playoffs <- test_five_playoffs %>% filter(metric == "ast_reb") %>% select(!metric) %>% mutate(Type = "Last 5")
pt_ast_five_playoffs <- test_five_playoffs %>% filter(metric == "pts_ast") %>% select(!metric) %>% mutate(Type = "Last 5")
stl_blk_five_playoffs <- test_five_playoffs %>% filter(metric == "stl_blk") %>% select(!metric) %>% mutate(Type = "Last 5")
fg3m_five_playoffs <- test_five_playoffs %>% filter(metric == "fg3m") %>% select(!metric) %>% mutate(Type = "Last 5")
stl_five_playoffs <- test_five_playoffs %>% filter(metric == "stl") %>% select(!metric) %>% mutate(Type = "Last 5")
blk_five_playoffs <- test_five_playoffs %>% filter(metric == "blk") %>% select(!metric) %>% mutate(Type = "Last 5")
tov_five_playoffs <- test_five_playoffs %>% filter(metric == "tov") %>% select(!metric) %>% mutate(Type = "Last 5")
pts_five_playoffs <- test_five_playoffs %>% filter(metric == "pts") %>% select(!metric) %>% mutate(Type = "Last 5")
ast_five_playoffs <- test_five_playoffs %>% filter(metric == "ast") %>% select(!metric) %>% mutate(Type = "Last 5")
treb_five_playoffs <- test_five_playoffs %>% filter(metric == "treb") %>% select(!metric) %>% mutate(Type = "Last 5")


test_ten_playoffs <- bind_rows(test_ten_playoffs)
test_2_ten_playoffs <- test_ten_playoffs %>% pivot_wider(names_from = OU, values_from = test)%>% unnest(cols = everything())

fg3a_ten_playoffs <- test_ten_playoffs %>% filter(metric == "fg3a") %>% select(!metric) %>% mutate(Type = "Last 10")
ftm_ten_playoffs <- test_ten_playoffs %>% filter(metric == "ftm") %>% select(!metric) %>% mutate(Type = "Last 10")
fgm_ten_playoffs <- test_ten_playoffs %>% filter(metric == "fgm") %>% select(!metric) %>% mutate(Type = "Last 10")
ptrebast_ten_playoffs <- test_ten_playoffs %>% filter(metric == "pts_reb_ast") %>% select(!metric) %>% mutate(Type = "Last 10")
pt_reb_ten_playoffs <- test_ten_playoffs %>% filter(metric == "pts_reb") %>% select(!metric) %>% mutate(Type = "Last 10")
ast_reb_ten_playoffs <- test_ten_playoffs %>% filter(metric == "ast_reb") %>% select(!metric) %>% mutate(Type = "Last 10")
pt_ast_ten_playoffs <- test_ten_playoffs %>% filter(metric == "pts_ast") %>% select(!metric) %>% mutate(Type = "Last 10")
stl_blk_ten_playoffs <- test_ten_playoffs %>% filter(metric == "stl_blk") %>% select(!metric) %>% mutate(Type = "Last 10")
fg3m_ten_playoffs <- test_ten_playoffs %>% filter(metric == "fg3m") %>% select(!metric) %>% mutate(Type = "Last 10")
stl_ten_playoffs <- test_ten_playoffs %>% filter(metric == "stl") %>% select(!metric) %>% mutate(Type = "Last 10")
blk_ten_playoffs <- test_ten_playoffs %>% filter(metric == "blk") %>% select(!metric) %>% mutate(Type = "Last 10")
tov_ten_playoffs <- test_ten_playoffs %>% filter(metric == "tov") %>% select(!metric) %>% mutate(Type = "Last 10")
pts_ten_playoffs <- test_ten_playoffs %>% filter(metric == "pts") %>% select(!metric) %>% mutate(Type = "Last 10")
ast_ten_playoffs <- test_ten_playoffs %>% filter(metric == "ast") %>% select(!metric) %>% mutate(Type = "Last 10")
treb_ten_playoffs <- test_ten_playoffs %>% filter(metric == "treb") %>% select(!metric) %>% mutate(Type = "Last 10")


ptreb_ast_df_playoffs <- bind_rows(ptrebast_playoffs,ptrebast_away_playoffs,ptrebast_home_playoffs,ptrebast_five_playoffs,ptrebast_ten_playoffs)

ptreb_ast_df_playoffs$namePlayer <- stri_trans_general(str = ptreb_ast_df_playoffs$namePlayer, id = "Latin-ASCII")

pt_reb_df_playoffs <- bind_rows(pt_reb_playoffs,pt_reb_away_playoffs,pt_reb_home_playoffs,pt_reb_five_playoffs,pt_reb_ten_playoffs)

pt_reb_df_playoffs$namePlayer <- stri_trans_general(str = pt_reb_df_playoffs$namePlayer, id = "Latin-ASCII")

ast_reb_df_playoffs <- bind_rows(ast_reb_playoffs,ast_reb_away_playoffs,ast_reb_home_playoffs,ast_reb_five_playoffs,ast_reb_ten_playoffs)

ast_reb_df_playoffs$namePlayer <- stri_trans_general(str = ast_reb_df_playoffs$namePlayer, id = "Latin-ASCII")

pt_ast_df_playoffs <- bind_rows(pt_ast_playoffs,pt_ast_away_playoffs,pt_ast_home_playoffs,pt_ast_five_playoffs,pt_ast_ten_playoffs)

pt_ast_df_playoffs$namePlayer <- stri_trans_general(str = pt_ast_df_playoffs$namePlayer, id = "Latin-ASCII")

stl_blk_df_playoffs <- bind_rows(stl_blk_playoffs,stl_blk_away_playoffs,stl_blk_home_playoffs,stl_blk_five_playoffs,stl_blk_ten_playoffs)

stl_blk_df_playoffs$namePlayer <- stri_trans_general(str = stl_blk_df_playoffs$namePlayer, id = "Latin-ASCII")

fg3m_df_playoffs <- bind_rows(fg3m_playoffs,fg3m_away_playoffs,fg3m_home_playoffs,fg3m_five_playoffs,fg3m_ten_playoffs)

fg3m_df_playoffs$namePlayer <- stri_trans_general(str = fg3m_df_playoffs$namePlayer, id = "Latin-ASCII")

stl_df_playoffs <- bind_rows(stl_playoffs,stl_away_playoffs,stl_home_playoffs,stl_five_playoffs,stl_ten_playoffs)

stl_df_playoffs$namePlayer <- stri_trans_general(str = stl_df_playoffs$namePlayer, id = "Latin-ASCII")

blk_df_playoffs <- bind_rows(blk_playoffs,blk_away_playoffs,blk_home_playoffs,blk_five_playoffs,blk_ten_playoffs)

blk_df_playoffs$namePlayer <- stri_trans_general(str = blk_df_playoffs$namePlayer, id = "Latin-ASCII")

tov_df_playoffs <- bind_rows(tov_playoffs,tov_away_playoffs,tov_home_playoffs,tov_five_playoffs,tov_ten_playoffs)

tov_df_playoffs$namePlayer <- stri_trans_general(str = tov_df_playoffs$namePlayer, id = "Latin-ASCII")

pts_df_playoffs <- bind_rows(pts_playoffs,pts_away_playoffs,pts_home_playoffs,pts_five_playoffs,pts_ten_playoffs)

pts_df_playoffs$namePlayer <- stri_trans_general(str = pts_df_playoffs$namePlayer, id = "Latin-ASCII")

ast_df_playoffs <- bind_rows(ast_playoffs,ast_away_playoffs,ast_home_playoffs,ast_five_playoffs,ast_ten_playoffs)

ast_df_playoffs$namePlayer <- stri_trans_general(str = ast_df_playoffs$namePlayer, id = "Latin-ASCII")

treb_df_playoffs <- bind_rows(treb_playoffs,treb_away_playoffs,treb_home_playoffs,treb_five_playoffs,treb_ten_playoffs)

treb_df_playoffs$namePlayer <- stri_trans_general(str = treb_df_playoffs$namePlayer, id = "Latin-ASCII")
...
firstqpoints_playoffs <- lapply(next_team_batch$idPlayer, function(x){
  slug_team <- all_rosters %>% filter(idPlayer == x) %>% pull(slugTeam)
  ...
  firstq_makes <- playerdata %>% filter(idPlayer == x, typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(dateGame,locationGame,typeSeason,slugOpponent,slugSeason,namePlayer,idPlayer,slugTeam) %>% summarize(n = n()) %>% left_join(play_play_makes %>% filter(numberPeriod == 1) %>% group_by(dateGame) %>% summarize(pts = sum(pts)), by = "dateGame") %>% mutate(pts = replace_na(pts,0))
...
firstqpoints_playoffs <- bind_rows(firstqpoints_playoffs) %>% pivot_wider(names_from = OU, values_from = test)%>% unnest(cols = everything())

firstqpoints_pivoted_playoffs <- firstqpoints_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                                     summarize(GP = n()), by = "idPlayer")  %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam)   %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 
...
firstqassists_playoffs <- lapply(next_team_batch$idPlayer, function(x){
  slug_team <- all_rosters %>% filter(idPlayer == x) %>% pull(slugTeam)
  ...
  firstq_assists <- playerdata %>% filter(idPlayer == x, typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(dateGame,locationGame,typeSeason,slugOpponent,slugSeason, namePlayer,idPlayer,slugTeam) %>% summarize(n = n()) %>% left_join(play_play_assists %>% filter(numberPeriod == 1) %>% group_by(dateGame,typeSeason,locationGame) %>% summarize(assists = n()), by = "dateGame") %>% mutate(assists = replace_na(assists,0)) %>% rename(locationGame = locationGame.x, typeSeason = typeSeason.x)
...
firstqassists_playoffs <- bind_rows(firstqassists_playoffs) %>% pivot_wider(names_from = OU, values_from = test)%>% unnest(cols = everything())

firstqassists_pivoted_playoffs <- firstqassists_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                                       summarize(GP = n()), by = "idPlayer")  %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam)   %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 
...
firstqrebounds_playoffs <- lapply(next_team_batch$idPlayer, function(x){
  slug_team <- all_rosters %>% filter(idPlayer == x) %>% pull(slugTeam)
  ...
  firstq_rebounds <- playerdata %>% filter(idPlayer == x, typeSeason == "Playoffs", slugSeason == "2024-25")  %>% 
    group_by(dateGame,locationGame,typeSeason,slugOpponent,slugSeason,namePlayer,idPlayer,slugTeam) %>% summarize(n = n()) %>% 
    left_join(play_play_rebounds %>% filter(numberPeriod == 1) %>% group_by(dateGame,typeSeason,locationGame) %>% summarize(rebounds = n()), by = "dateGame") %>%
    mutate(rebounds = replace_na(rebounds,0)) %>% rename(locationGame = locationGame.x, typeSeason = typeSeason.x)
...
firstqrebounds_playoffs <- bind_rows(firstqrebounds_playoffs) %>% pivot_wider(names_from = OU, values_from = test)%>% unnest(cols = everything())

firstqrebounds_pivoted_playoffs <- firstqrebounds_playoffs %>% left_join(playerdata %>% filter(typeSeason == "Playoffs", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
                                                         summarize(GP = n()), by = "idPlayer")  %>% 
  left_join(teams, by = "slugTeam") %>% rename(Player = namePlayer, Team = slugTeam)   %>% select(!c(idPlayer,Team,Opponent,idTeam,nameTeam)) %>% 
  relocate(urlThumbnailTeam, .after = Player) %>% relocate(GP, .after = urlThumbnailTeam) 
