...
next_opponent <- next_opponent_table %>% filter(Date == next_opponent_table %>% arrange(Date) %>% filter(!str_detect(Time,"Postponed"),!str_detect(Time,"-"),!str_detect(TV,"-"), !str_detect(TV,"Postponed")) %>% head(1) %>% pull(Date)) %>% pull(slugTeam)
