...
ptreb_ast_df_new <- ptreb_ast_df %>% filter(typeSeason == "Regular Season") %>% left_join(dk_ptrebast, by = c("namePlayer","OU")) %>% filter(!is.na(Over)) %>% rename(season_hit = test) %>% 
  ...
  left_join(ptreb_ast_df %>% filter(Type == "Regular Season",typeSeason == "Regular Season") %>% group_by(idPlayer) %>% summarize(variation_regular = mean(sd)), by = "idPlayer") %>%
  left_join(ptreb_ast_df %>% filter(Type == "Home Games",typeSeason == "Regular Season") %>% group_by(idPlayer) %>% summarize(variation_home = mean(sd)), by = "idPlayer") %>%
  left_join(ptreb_ast_df %>% filter(Type == "Away Games",typeSeason == "Regular Season") %>% group_by(idPlayer) %>% summarize(variation_away = mean(sd)), by = "idPlayer") %>%
  left_join(ptreb_ast_df %>% filter(Type == "Last 10",typeSeason == "Regular Season") %>% group_by(idPlayer) %>% summarize(variation_ten = mean(sd)), by = "idPlayer") %>%
  left_join(ptreb_ast_df %>% filter(Type == "Last 5",typeSeason == "Regular Season") %>% group_by(idPlayer) %>% summarize(variation_five = mean(sd)), by = "idPlayer")
...
pt_reb_df_new <- pt_reb_df %>% filter(typeSeason == "Regular Season") %>% left_join(dk_ptreb, by = c("namePlayer","OU")) %>% filter(!is.na(Over)) %>% rename(season_hit = test) %>% 
  left_join(playerdata %>% filter(typeSeason == "Regular Season", slugSeason == "2024-25") %>% 
              mutate(avg = pts+treb) %>% group_by(idPlayer) %>% summarize(avg = mean(avg), GP = n()), by = "idPlayer") %>% 
  left_join(pt_reb_df %>% filter(Type == "Regular Season", typeSeason == "Regular Season") %>% group_by(idPlayer) %>% summarize(variation_regular = mean(sd)), by = "idPlayer")
...
ast_reb_df_new <- ast_reb_df %>% filter(typeSeason == "Regular Season",typeSeason == "Regular Season") %>% left_join(dk_astreb, by = c("namePlayer","OU")) %>% filter(!is.na(Over)) %>% rename(season_hit = test) %>% 
  left_join(playerdata %>% filter(typeSeason == "Regular Season", slugSeason == "2024-25") %>% 
              mutate(avg = ast+treb) %>% group_by(idPlayer) %>% summarize(avg = mean(avg), GP = n()), by = "idPlayer") %>% 
  left_join(ast_reb_df %>% filter(Type == "Regular Season",typeSeason == "Regular Season") %>% group_by(idPlayer) %>% summarize(variation_regular = mean(sd)), by = "idPlayer")
...
stl_blk_df_new <- stl_blk_df %>% filter(typeSeason == "Regular Season",typeSeason == "Regular Season") %>% left_join(dk_stlblk, by = c("namePlayer","OU")) %>% filter(!is.na(Over)) %>% rename(season_hit = test) %>% 
  left_join(playerdata %>% filter(typeSeason == "Regular Season", slugSeason == "2024-25") %>% 
              mutate(avg = stl+blk) %>% group_by(idPlayer) %>% summarize(avg = mean(avg), GP = n()), by = "idPlayer") %>% 
  left_join(stl_blk_df %>% filter(Type == "Regular Season",typeSeason == "Regular Season") %>% group_by(idPlayer) %>% 
...
fg3m_df_new <- fg3m_df %>% filter(typeSeason == "Regular Season",typeSeason == "Regular Season") %>% left_join(dk_fg3m, by = c("namePlayer","OU")) %>% filter(!is.na(Over)) %>% rename(season_hit = test) %>% 
  ...
  left_join(fg3m_df %>% filter(Type == "Regular Season",typeSeason == "Regular Season") %>% group_by(idPlayer) %>% summarize(variation_regular = mean(sd)), by = "idPlayer")
...
stl_df_new <- stl_df %>% filter(typeSeason == "Regular Season",typeSeason == "Regular Season") %>% left_join(dk_stl, by = c("namePlayer","OU")) %>% filter(!is.na(Over)) %>% rename(season_hit = test) %>% 
  left_join(playerdata %>% filter(typeSeason == "Regular Season", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
              summarize(avg = mean(stl), GP = n()), by = "idPlayer") %>% 
  left_join(stl_df %>% filter(Type == "Regular Season",typeSeason == "Regular Season") %>% group_by(idPlayer) %>% summarize(variation_regular = mean(sd)), by = "idPlayer")
...
blk_df_new <- blk_df %>% filter(typeSeason == "Regular Season",typeSeason == "Regular Season") %>% left_join(dk_blk, by = c("namePlayer","OU")) %>% filter(!is.na(Over)) %>% rename(season_hit = test) %>% 
  left_join(playerdata %>% filter(typeSeason == "Regular Season", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
              summarize(avg = mean(blk), GP = n()), by = "idPlayer") %>% 
  left_join(blk_df %>% filter(Type == "Regular Season",typeSeason == "Regular Season") %>% group_by(idPlayer) %>% summarize(variation_regular = mean(sd)), by = "idPlayer")
...
tov_df_new <- tov_df %>% filter(typeSeason == "Regular Season",typeSeason == "Regular Season") %>% left_join(dk_tov, by = c("namePlayer","OU")) %>% filter(!is.na(Over)) %>% rename(season_hit = test) %>% 
  left_join(playerdata %>% filter(typeSeason == "Regular Season", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
              summarize(avg = mean(tov), GP = n()), by = "idPlayer") %>% 
  left_join(tov_df %>% filter(Type == "Regular Season",typeSeason == "Regular Season") %>% group_by(idPlayer) %>% summarize(variation_regular = mean(sd)), by = "idPlayer")
...
pts_df_new <- pts_df %>% filter(typeSeason == "Regular Season",typeSeason == "Regular Season") %>% left_join(dk_pts, by = c("namePlayer","OU")) %>% filter(!is.na(Over)) %>% rename(season_hit = test) %>% 
  left_join(playerdata %>% filter(typeSeason == "Regular Season", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
              summarize(avg = mean(pts), GP = n()), by = "idPlayer") %>% 
  left_join(pts_df %>% filter(Type == "Regular Season",typeSeason == "Regular Season") %>% group_by(idPlayer) %>% summarize(variation_regular = mean(sd)), by = "idPlayer")
...
ast_df_new <- ast_df %>% filter(typeSeason == "Regular Season",typeSeason == "Regular Season") %>% left_join(dk_ast, by = c("namePlayer","OU")) %>% filter(!is.na(Over)) %>% rename(season_hit = test) %>% 
  left_join(playerdata %>% filter(typeSeason == "Regular Season", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
              summarize(avg = mean(ast), GP = n()), by = "idPlayer") %>% 
  left_join(ast_df %>% filter(Type == "Regular Season",typeSeason == "Regular Season") %>% group_by(idPlayer) %>% summarize(variation_regular = mean(sd)), by = "idPlayer")
...
treb_df_new <- treb_df %>% filter(typeSeason == "Regular Season",typeSeason == "Regular Season") %>% left_join(dk_reb, by = c("namePlayer","OU")) %>% filter(!is.na(Over)) %>% rename(season_hit = test) %>% 
  left_join(playerdata %>% filter(typeSeason == "Regular Season", slugSeason == "2024-25") %>% group_by(idPlayer) %>% 
              summarize(avg = mean(treb), GP = n()), by = "idPlayer") %>% 
  left_join(treb_df %>% filter(Type == "Regular Season",typeSeason == "Regular Season") %>% group_by(idPlayer) %>% summarize(variation_regular = mean(sd)), by = "idPlayer")
