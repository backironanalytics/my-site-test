...
play_play_assists <- play_play_player_assists %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,namePlayer2,slugTeamPlayer1) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam"))
...
play_play_rebounds <- play_play_player_rebounds %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,slugTeamPlayer1) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) 
...
quarterly_ppg <- quarterly_points  %>% mutate(ppg = pts/n) %>% filter(typeSeason == "Regular Season", numberPeriod <= 4) %>% select(numberPeriod,ppg)
...
```{r Quarterly assists, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}
  ...
  quarterly_assists <- play_play_player_assists %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,namePlayer2,slugTeamPlayer1,idPlayerNBA1,idPlayerNBA2) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer2,idPlayerNBA2,slugTeam,numberPeriod,typeSeason) %>% summarize(assists = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA2) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason"))
...
quarterly_rebounds <-play_play_player_rebounds %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason) %>% summarize(reb = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA1) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason"))
...
p <- play_play_all_rebounds %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason) %>% summarize(reb = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA1) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason")) %>% filter(!is.na(slugTeam), typeSeason == "Regular Season", numberPeriod <= 4) %>% mutate(rpg = round(reb/n,1)) %>% ggplot(aes(rpg)) +geom_histogram(fill = "yellow", bins = 25, color = "grey") + facet_wrap(~numberPeriod, scales = 'free') + theme_minimal() + geom_vline(data = quarterly_rpg, aes(xintercept = rpg, color = "blue"),linetype = "dashed") + theme(legend.position = "none") + scale_x_continuous(breaks = seq(0,20,1)) + ggtitle("Avg Rebounds Per Quarter Per Game Distribution (OT Excluded)")

ggplotly(p)

percentile_rebounds_q1 <- play_play_all_rebounds %>% filter(is.na(slugScore)) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason) %>% summarize(reb = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA1) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason")) %>% filter(!is.na(slugTeam), typeSeason == "Regular Season", numberPeriod <= 4) %>% mutate(rpg = round(reb/n,1)) %>% filter(numberPeriod == 1) %>% mutate(percentile = percent_rank(rpg)*100) %>% filter(idPlayer == player_id[1]) %>% pull(percentile)
...
first <- quarterly_points %>% group_by(namePlayer1,idPlayer,numberPeriod,typeSeason) %>% summarize(pts = sum(pts), n = sum(n)) %>% ungroup()  %>% mutate(ppg = pts/n) %>% filter(typeSeason == "Regular Season", numberPeriod == 1) %>% pull(ppg)
zero <- 0
...
quarterly_apg <- play_play_player_assists %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,namePlayer2,slugTeamPlayer1,idPlayerNBA1,idPlayerNBA2) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer2,idPlayerNBA2,slugTeam,numberPeriod,typeSeason) %>% summarize(assists = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA2) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason")) %>% group_by(namePlayer2,idPlayer,numberPeriod,typeSeason) %>% summarize(assists = sum(assists), n = sum(n))  %>% mutate(apg = assists/n) %>% filter(typeSeason == "Regular Season", numberPeriod <= 4) %>% select(numberPeriod, apg)

first <- play_play_player_assists %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,namePlayer2,slugTeamPlayer1,idPlayerNBA1,idPlayerNBA2) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer2,idPlayerNBA2,slugTeam,numberPeriod,typeSeason) %>% summarize(assists = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA2) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason")) %>% group_by(namePlayer2,idPlayer,numberPeriod,typeSeason) %>% summarize(assists = sum(assists), n = sum(n))  %>% mutate(apg = assists/n) %>% filter(typeSeason == "Regular Season", numberPeriod == 1) %>% pull(apg)
zero <- 0
...
games <- play_play_player_assists %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,namePlayer2,slugTeamPlayer1,idPlayerNBA1,idPlayerNBA2) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer2,idPlayerNBA2,slugTeam,numberPeriod,typeSeason,dateGame,idGame) %>% summarize(assists = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA2) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason")) %>% group_by(namePlayer2,idPlayer,numberPeriod,typeSeason,slugTeam,dateGame,idGame) %>% summarize(assists = sum(assists)) %>% filter(numberPeriod == 1, typeSeason == "Regular Season", assists >= first) %>% group_by(idGame,dateGame) %>% summarize(firstq_assists = sum(assists)) %>% mutate(Type = "First Quarter")

games_firstqast <- games

all <- play_play_player_assists %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,namePlayer2,slugTeamPlayer1,idPlayerNBA1,idPlayerNBA2) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer2,idPlayerNBA2,slugTeam,numberPeriod,typeSeason,dateGame,idGame) %>% summarize(assists = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA2) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason")) %>% group_by(namePlayer2,idPlayer,numberPeriod,typeSeason,slugTeam,dateGame,idGame) %>% summarize(assists = sum(assists)) %>% filter(typeSeason == "Regular Season") %>% group_by(idGame,dateGame) %>% summarize(assists = sum(assists)) %>% mutate(Type = "Total")
...
all <- play_play_player_rebounds %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason,dateGame,idGame) %>% summarize(reb = sum(n)) %>% filter(typeSeason == "Regular Season") %>% group_by(idGame,dateGame) %>% summarize(reb = sum(reb)) %>% mutate(Type = "Total")
...
* In the regular season 1st Quarter, `r params$player` averaged **`r season$firstqppg`** points per game, **`r season$firstqapg`** assists per game, **`r season$firstqrpg`** rebounds per game
    * When `r params$player` reaches his Q1 PPG avg of `r season$firstqppg`, his total PPG avg is **`r a_pts`**. When it is not reached, his total PPG avg is **`r a_less`**, a difference of **`r a_pts - a_less`** points. His 1st Quarter PPG average is in the top **`r round(percentile_points_q1[[1]],0)` percentile** of the league
