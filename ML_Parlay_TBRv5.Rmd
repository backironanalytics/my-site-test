...
team_logo <- team_colors %>% filter(idTeam == team_id_most_recent) %>% pull(urlThumbnailTeam)
...
league_leaders_reg_season <- metrics_leaders(seasons = 2024,  season_types = "Regular Season", modes = "PerGame", return_message = T, metric = "pts")
...
team_name <- playerdata %>% filter(idPlayer == player_id) %>% 
     select(dateGame, idGame, slugOpponent, idPlayer, namePlayer, fgm,fga,pts,treb,ast,stl,blk,fg3m,fg3a,tov,
            plusminus,minutes,locationGame, countDaysRestPlayer, isB2B, isB2BFirst, isB2BSecond, nameTeam) %>% group_by(nameTeam) %>% summarize(n = n()) %>% pull(nameTeam)
```
...
<center> <h2>Stats for the `r season[1]` Season</h2></center>

```{r Season Stats, echo = FALSE, warning = FALSE, message = FALSE, fig.align='left', fig.width = 9, error = TRUE}


pts_per_attempt <- as.data.frame(playerdata %>% filter(typeSeason == "Regular Season") %>% group_by(namePlayer,idPlayer,slugTeam) %>% summarize(n = n(), pts = sum(pts), fga = sum(fga), min = sum(minutes))  %>% mutate(points_per_attempt = (pts/fga))) %>% mutate(rank = order(order(points_per_attempt, decreasing = T))) %>% arrange(desc(points_per_attempt))
...
player_data <- playerdata %>% filter(idPlayer == player_id) %>% select(dateGame, idGame, slugOpponent, idPlayer, namePlayer, fgm,fga,pts,treb,ast,stl,blk,fg3m,fg3a,tov,
         plusminus,minutes,locationGame, countDaysRestPlayer, isB2B, isB2BFirst, isB2BSecond, slugSeason,typeSeason)
...
play_play_makes <- play_play %>% filter(idPlayerNBA1 == player_id[1], !is.na(slugScore)) %>% select(idGame,numberPeriod, descriptionPlayHome,descriptionPlayNeutral,descriptionPlayVisitor, namePlayer1,slugScore,scoreHome,scoreAway,slugTeamPlayer1) %>% rename(slugTeam = slugTeamPlayer1) %>% mutate(description = ifelse(is.na(descriptionPlayHome),ifelse(is.na(descriptionPlayNeutral),descriptionPlayVisitor,descriptionPlayNeutral),descriptionPlayHome)) %>% mutate(free_throw_flag = str_detect(description,"Free Throw"), three_point_flag = str_detect(description,"3PT")) %>% mutate(two_point_flag = ifelse(free_throw_flag == FALSE & three_point_flag == FALSE, TRUE,FALSE)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% mutate(pts = ifelse(free_throw_flag == TRUE,1,ifelse(two_point_flag== TRUE,2,3)),score_type = ifelse(free_throw_flag == TRUE,"Free Throw",ifelse(two_point_flag== TRUE,"2 pt","3 pt")))

play_play_makes_expanded <- play_play %>% filter(numberPeriod == 1, !is.na(slugScore)) %>% select(idGame,numberPeriod, descriptionPlayHome,descriptionPlayNeutral,descriptionPlayVisitor, namePlayer1, idPlayerNBA1, slugScore,scoreHome,scoreAway, slugTeamPlayer1) %>% rename(slugTeam = slugTeamPlayer1) %>% mutate(description = ifelse(is.na(descriptionPlayHome),ifelse(is.na(descriptionPlayNeutral),descriptionPlayVisitor,descriptionPlayNeutral),descriptionPlayHome)) %>% mutate(free_throw_flag = str_detect(description,"Free Throw"), three_point_flag = str_detect(description,"3PT")) %>% mutate(two_point_flag = ifelse(free_throw_flag == FALSE & three_point_flag == FALSE, TRUE,FALSE)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame, slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% mutate(pts = ifelse(free_throw_flag == TRUE,1,ifelse(two_point_flag== TRUE,2,3)),score_type = ifelse(free_throw_flag == TRUE,"Free Throw",ifelse(two_point_flag== TRUE,"2 pt","3 pt"))) %>% group_by(dateGame,namePlayer1,idPlayerNBA1) %>% summarize(pts = sum(pts))

firstq_makes <- player_data %>% group_by(dateGame,locationGame,typeSeason,slugOpponent) %>% summarize(n = n()) %>% left_join(play_play_makes %>% filter(numberPeriod == 1) %>% group_by(dateGame) %>% summarize(pts = sum(pts)), by = "dateGame") %>% mutate(pts = replace_na(pts,0))
...
play_play_assists <- play_play %>% filter(idPlayerNBA2 == player_id[1], str_detect(descriptionPlayHome,"AST") | str_detect(descriptionPlayVisitor,"AST")) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,namePlayer2,slugTeamPlayer1) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam"))

firstq_assists <- player_data %>% group_by(dateGame,locationGame,typeSeason,slugOpponent) %>% summarize(n = n()) %>% left_join(play_play_assists %>% filter(numberPeriod == 1) %>% group_by(dateGame,typeSeason,locationGame) %>% summarize(assists = n()), by = "dateGame") %>% mutate(assists = replace_na(assists,0)) %>% rename(locationGame = locationGame.x, typeSeason = typeSeason.x)

play_play_rebounds <- play_play %>% filter(is.na(slugScore),idPlayerNBA1 == player_id[1], str_detect(descriptionPlayHome,"REBOUND") | str_detect(descriptionPlayVisitor,"REBOUND")) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,slugTeamPlayer1) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) 

firstq_rebounds <- player_data %>% group_by(dateGame,locationGame,typeSeason,slugOpponent) %>% summarize(n = n()) %>% left_join(play_play_rebounds %>% filter(numberPeriod == 1) %>% group_by(dateGame,typeSeason,locationGame) %>% summarize(rebounds = n()), by = "dateGame") %>% mutate(rebounds = replace_na(rebounds,0)) %>% rename(locationGame = locationGame.x, typeSeason = typeSeason.x)
...
season <- player_data %>% left_join(play_play_rebounds %>% filter(numberPeriod == 1) %>% group_by(idGame) %>% summarize(firstqrebounds = n()), by = "idGame") %>% mutate(firstqrebounds = replace_na(firstqrebounds,0)) %>% left_join(play_play_assists %>% filter(numberPeriod == 1) %>% group_by(idGame) %>% summarize(firstqassists = n()), by = "idGame") %>% mutate(firstqassists = replace_na(firstqassists,0)) %>% left_join(play_play_makes %>% filter(numberPeriod == 1) %>% group_by(idGame,namePlayer1,slugScore,scoreHome,scoreAway,score_type) %>% summarize(n = n(),firstqpoints = sum(pts)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason) %>% summarize(n = n()), by = "idGame") %>% group_by(idGame) %>% summarize(firstqpoints = sum(firstqpoints)), by = "idGame") %>% mutate(firstqpoints = replace_na(firstqpoints,0)) %>% left_join(play_play_makes %>% group_by(idGame,namePlayer1,slugScore,scoreHome,scoreAway,score_type) %>% summarize(n = n()) %>% mutate(first = ifelse((scoreHome >0 & scoreAway == 0) | (scoreAway > 0 & scoreHome == 0),1,0))%>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason) %>% summarize(n = n()), by = "idGame") %>% group_by(idGame,first) %>% summarize(n = n()) %>% filter(first == 1), by = "idGame") %>% mutate(first = replace_na((first),0)) %>% filter(typeSeason == "Regular Season") %>% group_by(namePlayer,slugSeason) %>% summarize(n = n(), total_pts = sum(pts), total_reb = sum(treb), total_ast = sum(ast), total_stl = sum(stl), total_blk = sum(blk), total_fgm = sum(fgm), total_fga = sum(fga), total_fg3m = sum(fg3m), total_fg3a = sum(fg3a), total_minutes = sum(minutes), total_tov = sum(tov), total_first = sum(first), totalfqpts = sum(firstqpoints), totalfqast = sum(firstqassists), totalfqtreb = sum(firstqrebounds)) %>% mutate(ppg = round(total_pts/n,1), rpg = round(total_reb/n,1), apg = round(total_ast/n,1), spg = round(total_stl/n,1), bpg = round(total_blk/n,1), fg_percentage = round((total_fgm/total_fga)*100,1), fg3_percentage = round((total_fg3m/total_fg3a)*100,1), min = round(total_minutes/n,1),threepg = round(total_fg3m/n,1), tovpg = round(total_tov/n,1), pts_ast = round((total_pts+total_ast)/n,1), pts_reb_ast = round((total_reb+total_pts+total_ast)/n,1), pts_reb = round((total_pts+total_reb)/n,1), ast_reb = round((total_ast+total_reb)/n,1),firstqppg = round(totalfqpts/n,1), firstqapg = round(totalfqast/n,1), firstqrpg = round(totalfqtreb/n,1), pts_per_attempt = round(total_pts/total_fga,2)) %>% select(n,min,ppg,rpg,apg,spg,bpg,threepg,fg_percentage,fg3_percentage,tovpg, pts_reb_ast, pts_ast,pts_reb,ast_reb, total_first,firstqppg,firstqapg,firstqrpg, pts_per_attempt) %>%rename(GP = n,`FG` = fg_percentage, `FG_3P` = fg3_percentage)
...
Type <- c("Regular Season","Last 10 GP","Last 5 GP","Regular Season Home","Regular Season Away",paste("Against",params$opponent,"Regular Season"),"Second Night of Back of Back",paste(playerdata$slugSeason[1],"Playoffs"),paste(playerdata$slugSeason[1],"Playoffs Away"),paste(playerdata$slugSeason[1],"Playoffs Home"))
combined <- bind_rows(season_frame,season_last10_frame,season_last5_frame,season_home_frame,season_away_frame,season_opponent_frame,b2b_frame,playoff_frame, playoff_away_frame,playoff_home_frame)


df <- data.frame(Type,combined)

df2 <- df %>% select(Type,GP,min,ppg,rpg,apg,spg,bpg,tovpg,threepg,FG,FG_3P,pts_reb_ast,pts_ast,pts_reb,ast_reb,total_first,firstqppg,firstqapg,firstqrpg,pts_per_attempt) %>% 
  rename(Min = min,
         PPG = ppg,
         RPG = rpg,
         APG = apg,
...
formattable(df2,align = "c",list(Min = color_tile("white","gold"),PPG = color_tile("white","gold"),RPG = color_tile("white","gold"),APG = color_tile("white","gold"),SPG = color_tile("white","gold"),BPG = color_tile("white","gold"),TOV = color_tile("white","gold"),FG3M = color_tile("white","gold"),`FG%` = color_tile("white","gold"),`3FG%`= color_tile("white","gold"), `Pts+Reb+Ast` = color_tile("white","gold"), `Pts+Ast` = color_tile("white","gold"), `Pts+Reb` = color_tile("white","gold"), `Reb+Ast` = color_tile("white","gold"), `First Scorer Count` = color_tile("white","gold"), `Q1 PPG` = color_tile("white","gold"), `Q1 APG` = color_tile("white","gold"), `Q1 RPG` = color_tile("white","gold"), `Pts Per Attempt` = color_tile("white","gold")), caption = "First Scorer Count Includes Free Throws. Second Night of Back to Back represents back to back for the team")
...
p <- player_data %>% filter(typeSeason == "Regular Season") %>% ggplot(aes(.data[[metric]])) + geom_histogram(bins = 10, color = secondary_color, fill = primary_color)+ scale_x_continuous(breaks = seq(0,90,2))+ ggtitle(toupper(paste(params$player,"Regular Season",metric,"histogram"))) + theme_minimal()+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

ggplotly(p) %>% layout(title = list(text = toupper(paste(params$player,metric,"histogram",'<br>','<sup>',toupper("Regular Season"),'</sup>'))))
...
avg_home <- mean(player_data %>% filter(typeSeason == "Regular Season", locationGame == "H") %>% pull(metric))
...
avg_away <- mean(player_data %>% filter(typeSeason == "Regular Season", locationGame == "A") %>% pull(metric))

valueBox(round(avg_away,1), icon = "fa-solid fa-basketball")
```


### Points per Game Against Next Opponent: `r params$opponent`

```{r}

avg_opp <- mean(player_data %>% filter(typeSeason == "Regular Season", slugOpponent == params$opponent) %>% pull(metric))
...
valueBox(round(message,1), icon = "fa-solid fa-basketball")
```
...
prediction <- if(params$location == "H"){
  (avg * avg_weight) + (avg_lastx * last_5_weight) + (avg_home * location_weight) + (avg_opp * opp_weight)
} else{
  (avg * avg_weight) + (avg_lastx * last_5_weight) + (avg_away * location_weight) + (avg_opp * opp_weight)
}
...
avg <- mean(player_data %>% filter(typeSeason == "Regular Season") %>% pull(metric))
avg_lastx <- mean(player_data %>% arrange(desc(dateGame)) %>% head(params$games) %>% pull(metric))
avg_opponent <- mean(player_data %>% filter(typeSeason == "Regular Season",slugOpponent == params$opponent) %>% pull(metric))
...
p <- player_data %>% filter(typeSeason == "Regular Season") %>% ggplot(aes(.data[[metric]])) + geom_histogram(bins = 10, color = secondary_color, fill = primary_color)+ theme_minimal() + scale_x_continuous(breaks = seq(0,90,2))+ ggtitle(toupper(paste(params$player,"Regular Season",metric,"histogram"))) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
p <- player_data %>% filter(typeSeason == "Regular Season") %>% ggplot(aes(locationGame,.data[[metric]], fill = locationGame)) + geom_boxplot() + theme_minimal()+ labs(x = "Location") + ggtitle(toupper(paste(params$player,"Regular Season",metric,"Per Game by location"))) + theme(legend.position="none")+ scale_fill_manual(values = c(primary_color,secondary_color))

ggplotly(p) %>% layout(title = list(text = toupper(paste(params$player,metric,"per game by location",'<br>','<sup>',toupper("Regular Season"),'</sup>'))))
...
p <- player_data %>% filter(typeSeason == "Regular Season") %>% mutate(day = weekdays(dateGame, abbreviate = TRUE))  %>% ggplot(aes(x=factor(day, levels = c("Sun","Mon","Tue","Wed","Thu","Fri","Sat")),y =.data[[metric]], fill = day)) + geom_boxplot() + theme_minimal()+ labs(x = "Weekday")  + theme(legend.position="none") + scale_fill_brewer(palette = "RdYlBu")

ggplotly(p) %>% layout(title = list(text = toupper(paste(params$player,metric,"per game by Weekday",'<br>','<sup>',toupper("Regular Season"),'</sup>'))))
...
player_data_regular <- player_data %>% filter(typeSeason == "Regular Season")
...
### Assists per Game Against Next Opponent: `r params$opponent`
...
p <- player_data %>% filter(typeSeason == "Regular Season") %>% ggplot(aes(.data[[metric]])) + geom_histogram(bins = 10, color = secondary_color, fill = primary_color)+ theme_minimal() + scale_x_continuous(breaks = seq(0,90,1))+ ggtitle(toupper(paste(params$player,"regular season",metric,"histogram")))
...
ggplotly(p, tooltip = list("OU","percentage","type"))  %>% layout(title = list(text = toupper(paste(params$player,metric,"O/U Probabilities",'<br>','<sup>',toupper("Regular Season"),'</sup>'))))
...
### Rebounds per Game Against Next Opponent: `r params$opponent`
...
### Steals per Game Against Next Opponent: `r params$opponent`
...
p <- player_data %>% filter(typeSeason == "Regular Season") %>% ggplot(aes(.data[[metric]])) + geom_histogram(bins = 6, color = secondary_color, fill = primary_color)+ theme_minimal() + scale_x_continuous(breaks = seq(0,90,1))+ ggtitle(toupper(paste(params$player,"regular season",metric,"histogram")))
...
Three Pointers {data-navmenu="Exploratory Stats"}
=======================================================================
...
### Three's Made per Game Against Next Opponent: `r params$opponent`
...
### Turnovers per Game Against Next Opponent: `r params$opponent`
...
### Blocks per Game Against Next Opponent: `r params$opponent`
...
avg <- mean(dat %>% filter(typeSeason == "Regular Season") %>% pull(metric))
...
avg_home <- mean(dat %>% filter(typeSeason == "Regular Season", locationGame == "H") %>% pull(metric))
...
avg_away <- mean(dat %>% filter(typeSeason == "Regular Season", locationGame == "A") %>% pull(metric))

valueBox(round(avg_away,1), icon = "fa-solid fa-basketball")
```


### Pts+Reb+Ast per Game Against Next Opponent: `r params$opponent`

```{r}

avg_opp <- mean(dat %>% filter(typeSeason == "Regular Season", slugOpponent == params$opponent) %>% pull(metric))
...
dat_combined <- playerdata %>% filter(idPlayer == player_id) %>% 
  select(dateGame, idGame, slugOpponent, idPlayer, namePlayer, fgm,fga,pts,treb,ast,stl,blk,fg3m,fg3a,tov,
         plusminus,minutes,locationGame, countDaysRestPlayer, isB2B, isB2BFirst, isB2BSecond,typeSeason) %>% 
  arrange(desc(dateGame)) %>% mutate(tripdoub = ifelse(pts >= 10 & treb >= 10 & ast >= 10, "Y","N"),pt_reb_ast_total = pts+treb+ast) %>% select(dateGame,locationGame,pts,treb,ast,pt_reb_ast_total,typeSeason) 

avg <- mean(dat %>% filter(typeSeason == "Regular Season") %>% pull(pt_reb_ast_total))
avg_lastx <- mean(dat %>% head(params$games) %>% pull(pt_reb_ast_total))


season_rank <- playerdata %>% filter(typeSeason == "Regular Season")  %>% 
    select(dateGame, idGame, slugOpponent, idPlayer, namePlayer, fgm,fga,pts,treb,ast,stl,blk,fg3m,fg3a,tov,
           plusminus,minutes,locationGame, countDaysRestPlayer, isB2B, isB2BFirst, isB2BSecond,typeSeason) %>% 
...
ggplotly(p) %>% layout(legend = list(orientation = "h", x = 0, y = -0.2), 
                       title = list(text = toupper(paste(params$player,"Pts+reb+ast",'<br>','<sup>',toupper(paste("Season Avg:",round(avg,1),"/","Avg Last",params$games,"games:",round(avg_lastx,1))),'</sup>'))))
...
p <- dat %>% filter(typeSeason == "Regular Season") %>% ggplot(aes(locationGame,.data[[metric]], fill = locationGame)) + geom_boxplot() + theme_minimal()+ labs(x = "Location") + ggtitle(toupper(paste(params$player,"regular season",metric,"per game by location"))) + theme(legend.position="none") + scale_fill_manual(values = c(primary_color,secondary_color))
...
p <- dat %>% filter(typeSeason == "Regular Season") %>% mutate(day = weekdays(dateGame, abbreviate = TRUE))  %>% ggplot(aes(x=factor(day, levels = c("Sun","Mon","Tue","Wed","Thu","Fri","Sat")),y =.data[[metric]], fill = day)) + geom_boxplot() + theme_minimal()+ labs(x = "Weekday")  + theme(legend.position="none")+ scale_fill_brewer(palette = "RdYlBu")
...
p <- playerdata %>% filter(typeSeason == "Regular Season") %>% filter(idPlayer == player_id) %>% 
  select(dateGame, idGame, slugOpponent, idPlayer, namePlayer, fgm,fga,pts,treb,ast,stl,blk,fg3m,fg3a,tov,
         plusminus,minutes,locationGame, countDaysRestPlayer, isB2B, isB2BFirst, isB2BSecond) %>% 
...
player_data_regular <- dat %>% filter(typeSeason == "Regular Season")
...
### Pts+Ast per Game Against Next Opponent: `r params$opponent`
...
dat_combined <- playerdata %>% filter(idPlayer == player_id) %>% 
  select(dateGame, idGame, slugOpponent, idPlayer, namePlayer, fgm,fga,pts,treb,ast,stl,blk,fg3m,fg3a,tov,
         plusminus,minutes,locationGame, countDaysRestPlayer, isB2B, isB2BFirst, isB2BSecond,typeSeason) %>% 
  arrange(desc(dateGame)) %>% mutate(tripdoub = ifelse(pts >= 10 & treb >= 10 & ast >= 10, "Y","N"),pt_ast_total = pts+ast) %>% select(dateGame,locationGame,pts,treb,ast,pt_ast_total,typeSeason) 

avg <- mean(dat %>% filter(typeSeason == "Regular Season") %>% pull(pt_ast_total))
...
ggplotly(p) %>% layout(legend = list(orientation = "h", x = 0, y = -0.2), 
                       title = list(text = toupper(paste(params$player,"Pts+ast",'<br>','<sup>',toupper(paste("Season Avg:",round(avg,1),"/","Avg Last",params$games,"games:",round(avg_lastx,1))),'</sup>'))))
...
### Pts+Reb per Game Against Next Opponent: `r params$opponent`
...
dat_combined <- playerdata %>% filter(idPlayer == player_id) %>% 
  select(dateGame, idGame, slugOpponent, idPlayer, namePlayer, fgm,fga,pts,treb,ast,stl,blk,fg3m,fg3a,tov,
         plusminus,minutes,locationGame, countDaysRestPlayer, isB2B, isB2BFirst, isB2BSecond,typeSeason) %>% 
  arrange(desc(dateGame)) %>% mutate(tripdoub = ifelse(pts >= 10 & treb >= 10 & ast >= 10, "Y","N"),pt_reb_total = pts+treb) %>% select(dateGame,locationGame,pts,treb,ast,pt_reb_total,typeSeason) 

avg <- mean(dat %>% filter(typeSeason == "Regular Season") %>% pull(pt_reb_total))
...
ggplotly(p) %>% layout(legend = list(orientation = "h", x = 0, y = -0.2), 
                       title = list(text = toupper(paste(params$player,"Pts+reb",'<br>','<sup>',toupper(paste("Season Avg:",round(avg,1),"/","Avg Last",params$games,"games:",round(avg_lastx,1))),'</sup>'))))
...
### Reb+Ast per Game Against Next Opponent: `r params$opponent`
...
dat_combined <- playerdata %>% filter(idPlayer == player_id) %>% 
  select(dateGame, idGame, slugOpponent, idPlayer, namePlayer, fgm,fga,pts,treb,ast,stl,blk,fg3m,fg3a,tov,
         plusminus,minutes,locationGame, countDaysRestPlayer, isB2B, isB2BFirst, isB2BSecond,typeSeason) %>% 
  arrange(desc(dateGame)) %>% mutate(tripdoub = ifelse(pts >= 10 & treb >= 10 & ast >= 10, "Y","N"),reb_ast_total = ast+treb) %>% select(dateGame,locationGame,pts,treb,ast,reb_ast_total,typeSeason) 

avg <- mean(dat %>% filter(typeSeason == "Regular Season") %>% pull(reb_ast_total))
...
ggplotly(p) %>% layout(legend = list(orientation = "h", x = 0, y = -0.2), 
                       title = list(text = toupper(paste(params$player,"Reb+Ast",'<br>','<sup>',toupper(paste("Season Avg:",round(avg,1),"/","Avg Last",params$games,"games:",round(avg_lastx,1))),'</sup>'))))
...
avg <- mean(firstq_makes %>% filter(typeSeason == "Regular Season") %>% pull(metric))
...
avg_home <- mean(firstq_makes %>% filter(typeSeason == "Regular Season", locationGame == "H") %>% pull(metric))
...
avg_away <- mean(firstq_makes %>% filter(typeSeason == "Regular Season", locationGame == "A") %>% pull(metric))
...
avg_opp <- mean(firstq_makes %>% filter(typeSeason == "Regular Season", slugOpponent == params$opponent) %>% pull(metric))
...
percentile_points_q1 <- play_play %>% filter(!is.na(slugScore)) %>% select(idGame,numberPeriod, descriptionPlayHome,descriptionPlayNeutral,descriptionPlayVisitor, namePlayer1,slugScore,scoreHome,scoreAway,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% mutate(description = ifelse(is.na(descriptionPlayHome),ifelse(is.na(descriptionPlayNeutral),descriptionPlayVisitor,descriptionPlayNeutral),descriptionPlayHome)) %>% mutate(free_throw_flag = str_detect(description,"Free Throw"), three_point_flag = str_detect(description,"3PT")) %>% mutate(two_point_flag = ifelse(free_throw_flag == FALSE & three_point_flag == FALSE, TRUE,FALSE)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% mutate(pts = ifelse(free_throw_flag == TRUE,1,ifelse(two_point_flag== TRUE,2,3)),score_type = ifelse(free_throw_flag == TRUE,"Free Throw",ifelse(two_point_flag== TRUE,"2 pt","3 pt"))) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason) %>% summarize(pts = sum(pts)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA1) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason,slugTeam) %>% summarize(n = n()), by = c("idPlayer","typeSeason","slugTeam")) %>% filter(!is.na(slugTeam), typeSeason == "Regular Season", numberPeriod <= 4) %>% mutate(ppg = round(pts/n,1)) %>% filter(numberPeriod == 1) %>% mutate(percentile = percent_rank(ppg)*100) %>% filter(idPlayer == player_id[1]) %>% pull(percentile)
...
ggplotly(p) %>% layout(legend = list(orientation = "h", x = 0, y = -0.2), 
                       title = list(text = toupper(paste(params$player,"1st Quarter Pts",'<br>','<sup>',toupper(paste("Season Avg:",round(avg,1),"/","Avg Last",params$games,"games:",round(avg_lastx,1))),'</sup>'))))
...
p <- firstq_makes %>% filter(typeSeason == "Regular Season") %>% ggplot(aes(locationGame,.data[[metric]], fill = locationGame)) + geom_boxplot() + theme_minimal()+ labs(x = "Location") + ggtitle(toupper(paste(params$player,"regular season",metric,"per game by location"))) + theme(legend.position="none") + scale_fill_manual(values = c(primary_color,secondary_color))
...
p <- firstq_makes %>% filter(typeSeason == "Regular Season") %>% mutate(day = weekdays(dateGame, abbreviate = TRUE))  %>% ggplot(aes(x=factor(day, levels = c("Sun","Mon","Tue","Wed","Thu","Fri","Sat")),y =.data[[metric]], fill = day)) + geom_boxplot() + theme_minimal()+ labs(x = "Weekday")  + theme(legend.position="none")+ scale_fill_brewer(palette = "RdYlBu")
...
p <- firstq_makes %>% filter(typeSeason == "Regular Season") %>% ggplot(aes(.data[[metric]])) + geom_histogram(bins = 6, color = secondary_color, fill = primary_color)+ theme_minimal() + scale_x_continuous(breaks = seq(0,90,1))+ ggtitle(toupper(paste(params$player,"regular season",metric,"histogram")))
...
avg <- round(mean(attempt %>% filter(typeSeason == "Regular Season", numberPeriod == period) %>% pull(attempts_first)),1)
...
avg <- mean(firstq_assists %>% filter(typeSeason == "Regular Season") %>% pull(metric))
...
avg_lastx <- mean(firstq_assists %>% arrange(desc(dateGame)) %>% head(5) %>% pull(metric))
...
avg_home <- mean(firstq_assists %>% filter(typeSeason == "Regular Season", locationGame == "H") %>% pull(metric))
...
avg_away <- mean(firstq_assists %>% filter(typeSeason == "Regular Season", locationGame == "A") %>% pull(metric))
...
avg_opp <- mean(firstq_assists %>% filter(typeSeason == "Regular Season", slugOpponent == params$opponent) %>% pull(metric))
...
avg <- mean(firstq_assists %>% filter(typeSeason == "Regular Season") %>% pull(assists))
...
ggplotly(p) %>% layout(legend = list(orientation = "h", x = 0, y = -0.2), 
                       title = list(text = toupper(paste(params$player,"1st Quarter Assists",'<br>','<sup>',toupper(paste("Season Avg:",round(avg,1),"/","Avg Last",params$games,"games:",round(avg_lastx,1))),'</sup>'))))
...
p <- firstq_assists %>% filter(typeSeason == "Regular Season") %>% ggplot(aes(locationGame,.data[[metric]], fill = locationGame)) + geom_boxplot() + theme_minimal()+ labs(x = "Location") + ggtitle(toupper(paste(params$player,"regular season",metric,"per game by location"))) + theme(legend.position="none") + scale_fill_manual(values = c(primary_color,secondary_color))
...
p <- firstq_assists %>% filter(typeSeason == "Regular Season") %>% mutate(day = weekdays(dateGame, abbreviate = TRUE))  %>% ggplot(aes(x=factor(day, levels = c("Sun","Mon","Tue","Wed","Thu","Fri","Sat")),y =.data[[metric]], fill = day)) + geom_boxplot() + theme_minimal()+ labs(x = "Weekday")  + theme(legend.position="none")+ scale_fill_brewer(palette = "RdYlBu")
...
p <- firstq_assists %>% filter(typeSeason == "Regular Season") %>% ggplot(aes(.data[[metric]])) + geom_histogram(bins = 6, color = secondary_color, fill = primary_color)+ theme_minimal() + scale_x_continuous(breaks = seq(0,90,1))+ ggtitle(toupper(paste(params$player,"regular season",metric,"histogram")))
...
avg <- mean(firstq_rebounds %>% filter(typeSeason == "Regular Season") %>% pull(metric))
...
avg_lastx <- mean(firstq_rebounds %>% arrange(desc(dateGame)) %>% head(5) %>% pull(metric))
...
avg_home <- mean(firstq_rebounds %>% filter(typeSeason == "Regular Season", locationGame == "H") %>% pull(metric))
...
avg_away <- mean(firstq_rebounds %>% filter(typeSeason == "Regular Season", locationGame == "A") %>% pull(metric))
...
avg_opp <- mean(firstq_rebounds %>% filter(typeSeason == "Regular Season", slugOpponent == params$opponent) %>% pull(metric))
...
avg <- mean(firstq_rebounds %>% filter(typeSeason == "Regular Season") %>% pull(rebounds))
...
ggplotly(p) %>% layout(legend = list(orientation = "h", x = 0, y = -0.2), 
                       title = list(text = toupper(paste(params$player,"1st Quarter Rebounds",'<br>','<sup>',toupper(paste("Season Avg:",round(avg,1),"/","Avg Last",params$games,"games:",round(avg_lastx,1))),'</sup>'))))
...
p <- firstq_rebounds %>% filter(typeSeason == "Regular Season") %>% ggplot(aes(locationGame,.data[[metric]], fill = locationGame)) + geom_boxplot() + theme_minimal()+ labs(x = "Location") + ggtitle(toupper(paste(params$player,"regular season",metric,"per game by location"))) + theme(legend.position="none") + scale_fill_manual(values = c(primary_color,secondary_color))
...
p <- firstq_rebounds %>% filter(typeSeason == "Regular Season") %>% mutate(day = weekdays(dateGame, abbreviate = TRUE))  %>% ggplot(aes(x=factor(day, levels = c("Sun","Mon","Tue","Wed","Thu","Fri","Sat")),y =.data[[metric]], fill = day)) + geom_boxplot() + theme_minimal()+ labs(x = "Weekday")  + theme(legend.position="none")+ scale_fill_brewer(palette = "RdYlBu")
...
p <- firstq_rebounds %>% filter(typeSeason == "Regular Season") %>% ggplot(aes(.data[[metric]])) + geom_histogram(bins = 6, color = secondary_color, fill = primary_color)+ theme_minimal() + scale_x_continuous(breaks = seq(0,90,1))+ ggtitle(toupper(paste(params$player,"regular season",metric,"histogram")))
...
```{r rebounds probability, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", include = FALSE}

reb_over_under_new <- seq(from = 1, to = 9, by = 1)

prob <- sapply(reb_over_under_new, function (x){
  mean(firstq_rebounds$rebounds >= x)
})
...
### Regular Season Points per Quarter {data-width=1500}

```{r Quarterly points, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}


quarterly_points <-play_play %>% filter(idPlayerNBA1 == player_id[1], !is.na(slugScore)) %>% select(idGame,numberPeriod, descriptionPlayHome,descriptionPlayNeutral,descriptionPlayVisitor, namePlayer1,slugScore,scoreHome,scoreAway,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% mutate(description = ifelse(is.na(descriptionPlayHome),ifelse(is.na(descriptionPlayNeutral),descriptionPlayVisitor,descriptionPlayNeutral),descriptionPlayHome)) %>% mutate(free_throw_flag = str_detect(description,"Free Throw"), three_point_flag = str_detect(description,"3PT")) %>% mutate(two_point_flag = ifelse(free_throw_flag == FALSE & three_point_flag == FALSE, TRUE,FALSE)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% mutate(pts = ifelse(free_throw_flag == TRUE,1,ifelse(two_point_flag== TRUE,2,3)),score_type = ifelse(free_throw_flag == TRUE,"Free Throw",ifelse(two_point_flag== TRUE,"2 pt","3 pt"))) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason) %>% summarize(pts = sum(pts)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA1) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason,slugTeam) %>% summarize(n = n()), by = c("idPlayer","typeSeason","slugTeam"))

quarterly_points <- replace(quarterly_points,is.na(quarterly_points),0)

p <- quarterly_points %>% group_by(namePlayer1,idPlayer,numberPeriod,typeSeason) %>% summarize(pts = sum(pts), n = sum(n))  %>% mutate(ppg = pts/n) %>% filter(typeSeason == "Regular Season", numberPeriod <= 4) %>% ggplot(aes(numberPeriod,ppg)) + geom_point(size = 5, color = "orange")+ geom_segment(aes(x=numberPeriod,xend = numberPeriod, y = 0, yend = ppg), color = "grey")+ theme_minimal() + geom_text(aes(label = round(ppg,1)), position = position_dodge(width = 0.9),vjust = -2) + labs(x = "Quarter", y = "Avg Points") + ggtitle("Avg Points Per Quarter Per Game (OT Excluded)") + scale_y_continuous(breaks = seq(0,50,1)) + scale_color_manual(values = c(primary_color,secondary_color))
...
### Regular Season Points per Quarter League Comparison

```{r Quarterly points hist, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}


quarterly_ppg <- quarterly_points %>% group_by(namePlayer1,idPlayer,numberPeriod,typeSeason) %>% summarize(pts = sum(pts), n = sum(n))   %>% mutate(ppg = pts/n) %>% filter(typeSeason == "Regular Season", numberPeriod <= 4) %>% select(numberPeriod,ppg)

p <- play_play %>% filter(!is.na(slugScore)) %>% select(idGame,numberPeriod, descriptionPlayHome,descriptionPlayNeutral,descriptionPlayVisitor, namePlayer1,slugScore,scoreHome,scoreAway,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% mutate(description = ifelse(is.na(descriptionPlayHome),ifelse(is.na(descriptionPlayNeutral),descriptionPlayVisitor,descriptionPlayNeutral),descriptionPlayHome)) %>% mutate(free_throw_flag = str_detect(description,"Free Throw"), three_point_flag = str_detect(description,"3PT")) %>% mutate(two_point_flag = ifelse(free_throw_flag == FALSE & three_point_flag == FALSE, TRUE,FALSE)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% mutate(pts = ifelse(free_throw_flag == TRUE,1,ifelse(two_point_flag== TRUE,2,3)),score_type = ifelse(free_throw_flag == TRUE,"Free Throw",ifelse(two_point_flag== TRUE,"2 pt","3 pt"))) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason) %>% summarize(pts = sum(pts)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA1) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason,slugTeam) %>% summarize(n = n()), by = c("idPlayer","typeSeason","slugTeam")) %>% filter(!is.na(slugTeam), typeSeason == "Regular Season", numberPeriod <= 4) %>% mutate(ppg = round(pts/n,1)) %>% ggplot(aes(ppg)) +geom_histogram(fill = "orange", bins = 25, color = "grey") + facet_wrap(~numberPeriod, scales = 'free') + theme_minimal() + geom_vline(data = quarterly_ppg, aes(xintercept = ppg, color = "blue"),linetype = "dashed") + theme(legend.position = "none") + scale_x_continuous(breaks = seq(0,20,1)) + ggtitle("Avg Points Per Quarter Per Game Distribution (OT Excluded)")

ggplotly(p)



percentile_points_q2 <- play_play %>% filter(!is.na(slugScore)) %>% select(idGame,numberPeriod, descriptionPlayHome,descriptionPlayNeutral,descriptionPlayVisitor, namePlayer1,slugScore,scoreHome,scoreAway,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% mutate(description = ifelse(is.na(descriptionPlayHome),ifelse(is.na(descriptionPlayNeutral),descriptionPlayVisitor,descriptionPlayNeutral),descriptionPlayHome)) %>% mutate(free_throw_flag = str_detect(description,"Free Throw"), three_point_flag = str_detect(description,"3PT")) %>% mutate(two_point_flag = ifelse(free_throw_flag == FALSE & three_point_flag == FALSE, TRUE,FALSE)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% mutate(pts = ifelse(free_throw_flag == TRUE,1,ifelse(two_point_flag== TRUE,2,3)),score_type = ifelse(free_throw_flag == TRUE,"Free Throw",ifelse(two_point_flag== TRUE,"2 pt","3 pt"))) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason) %>% summarize(pts = sum(pts)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA1) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason,slugTeam) %>% summarize(n = n()), by = c("idPlayer","typeSeason","slugTeam")) %>% filter(!is.na(slugTeam), typeSeason == "Regular Season", numberPeriod <= 4) %>% mutate(ppg = round(pts/n,1)) %>% filter(numberPeriod == 2) %>% mutate(percentile = percent_rank(ppg)*100) %>% filter(idPlayer == player_id[1]) %>% pull(percentile)

percentile_points_q3 <- play_play %>% filter(!is.na(slugScore)) %>% select(idGame,numberPeriod, descriptionPlayHome,descriptionPlayNeutral,descriptionPlayVisitor, namePlayer1,slugScore,scoreHome,scoreAway,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% mutate(description = ifelse(is.na(descriptionPlayHome),ifelse(is.na(descriptionPlayNeutral),descriptionPlayVisitor,descriptionPlayNeutral),descriptionPlayHome)) %>% mutate(free_throw_flag = str_detect(description,"Free Throw"), three_point_flag = str_detect(description,"3PT")) %>% mutate(two_point_flag = ifelse(free_throw_flag == FALSE & three_point_flag == FALSE, TRUE,FALSE)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% mutate(pts = ifelse(free_throw_flag == TRUE,1,ifelse(two_point_flag== TRUE,2,3)),score_type = ifelse(free_throw_flag == TRUE,"Free Throw",ifelse(two_point_flag== TRUE,"2 pt","3 pt"))) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason) %>% summarize(pts = sum(pts)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA1) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason,slugTeam) %>% summarize(n = n()), by = c("idPlayer","typeSeason","slugTeam")) %>% filter(!is.na(slugTeam), typeSeason == "Regular Season", numberPeriod <= 4) %>% mutate(ppg = round(pts/n,1)) %>% filter(numberPeriod == 3) %>% mutate(percentile = percent_rank(ppg)*100) %>% filter(idPlayer == player_id[1]) %>% pull(percentile)

percentile_points_q4 <- play_play %>% filter(!is.na(slugScore)) %>% select(idGame,numberPeriod, descriptionPlayHome,descriptionPlayNeutral,descriptionPlayVisitor, namePlayer1,slugScore,scoreHome,scoreAway,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% mutate(description = ifelse(is.na(descriptionPlayHome),ifelse(is.na(descriptionPlayNeutral),descriptionPlayVisitor,descriptionPlayNeutral),descriptionPlayHome)) %>% mutate(free_throw_flag = str_detect(description,"Free Throw"), three_point_flag = str_detect(description,"3PT")) %>% mutate(two_point_flag = ifelse(free_throw_flag == FALSE & three_point_flag == FALSE, TRUE,FALSE)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% mutate(pts = ifelse(free_throw_flag == TRUE,1,ifelse(two_point_flag== TRUE,2,3)),score_type = ifelse(free_throw_flag == TRUE,"Free Throw",ifelse(two_point_flag== TRUE,"2 pt","3 pt"))) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason) %>% summarize(pts = sum(pts)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA1) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason,slugTeam) %>% summarize(n = n()), by = c("idPlayer","typeSeason","slugTeam")) %>% filter(!is.na(slugTeam), typeSeason == "Regular Season", numberPeriod <= 4) %>% mutate(ppg = round(pts/n,1)) %>% filter(numberPeriod == 4) %>% mutate(percentile = percent_rank(ppg)*100) %>% filter(idPlayer == player_id[1]) %>% pull(percentile)
...
### Regular Season Assists per Quarter {data-width=1500}

```{r Quarterly assists, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}

  quarterly_assists <- play_play %>% filter(idPlayerNBA2 == player_id[1], str_detect(descriptionPlayHome,"AST") | str_detect(descriptionPlayVisitor,"AST")) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,namePlayer2,slugTeamPlayer1,idPlayerNBA1,idPlayerNBA2) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer2,idPlayerNBA2,slugTeam,numberPeriod,typeSeason) %>% summarize(assists = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA2) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason"))
...
quarterly_assists <- lapply(periods, function(x){
  ...
  player_data %>% group_by(idPlayer,namePlayer) %>% summarize(n = n()) %>% mutate(numberPeriod = x, typeSeason = "Regular Season") %>% rename(namePlayer2 = namePlayer) %>% left_join(quarterly_assists, by = c("idPlayer","numberPeriod","typeSeason")) %>% mutate(assists = replace_na(assists,0)) %>% rename(namePlayer2 = namePlayer2.x) 
...
p <- quarterly_assists %>% group_by(namePlayer2,idPlayer,numberPeriod,typeSeason) %>% summarize(assists = sum(assists), n = sum(n.y))  %>% mutate(apg = assists/n) %>% filter(typeSeason == "Regular Season", numberPeriod <= 4) %>% ggplot(aes(numberPeriod,apg)) + geom_point(size = 5, color = "green")+ geom_segment(aes(x=numberPeriod,xend = numberPeriod, y = 0, yend = apg), color = "grey")+ theme_minimal() + geom_text(aes(label = round(apg,1)), position = position_dodge(width = 0.9),vjust = -2) + labs(x = "Quarter", y = "Avg Assists") + ggtitle("Avg Assists Per Quarter Per Game (OT Excluded)") + scale_y_continuous(breaks = seq(0,50,1))
...
### Regular Season Assists per Quarter League Comparison

```{r Quarterly assists hist, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}


quarterly_apg <- quarterly_assists %>% group_by(namePlayer2,idPlayer,numberPeriod,typeSeason) %>% summarize(assists = sum(assists), n = sum(n.y))  %>% mutate(apg = assists/n) %>% filter(typeSeason == "Regular Season", numberPeriod <= 4) %>% select(numberPeriod,apg)

p <- play_play %>% filter(str_detect(descriptionPlayHome,"AST") | str_detect(descriptionPlayVisitor,"AST")) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,namePlayer2,slugTeamPlayer1,idPlayerNBA1,idPlayerNBA2) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer2,idPlayerNBA2,slugTeam,numberPeriod,typeSeason) %>% summarize(assists = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA2) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason")) %>% filter(!is.na(slugTeam), typeSeason == "Regular Season", numberPeriod <= 4) %>% mutate(apg = round(assists/n,1)) %>% ggplot(aes(apg)) +geom_histogram(fill = "green", bins = 25, color = "grey") + facet_wrap(~numberPeriod, scales = 'free') + theme_minimal() + geom_vline(data = quarterly_apg, aes(xintercept = apg, color = "blue"),linetype = "dashed") + theme(legend.position = "none") + scale_x_continuous(breaks = seq(0,20,1)) + ggtitle("Avg Assists Per Quarter Per Game Distribution (OT Excluded)")

ggplotly(p)

percentile_assists_q1 <- play_play %>% filter(str_detect(descriptionPlayHome,"AST") | str_detect(descriptionPlayVisitor,"AST")) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,namePlayer2,slugTeamPlayer1,idPlayerNBA1,idPlayerNBA2) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer2,idPlayerNBA2,slugTeam,numberPeriod,typeSeason) %>% summarize(assists = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA2) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason")) %>% filter(!is.na(slugTeam), typeSeason == "Regular Season", numberPeriod <= 4) %>% mutate(apg = round(assists/n,1)) %>% filter(numberPeriod == 1) %>% mutate(percentile = percent_rank(apg)*100) %>% filter(idPlayer == player_id[1]) %>% pull(percentile)
...
### Regular Season Rebounds per Quarter {data-width=1500}

```{r Quarterly rebounds, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}


quarterly_rebounds <-play_play %>% filter(is.na(slugScore),idPlayerNBA1 == player_id[1], str_detect(descriptionPlayHome,"REBOUND") | str_detect(descriptionPlayVisitor,"REBOUND")) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason) %>% summarize(reb = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA1) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason"))

quarterly_rebounds <- replace(quarterly_rebounds,is.na(quarterly_rebounds),0)

p <- quarterly_rebounds %>% group_by(namePlayer1,idPlayer,numberPeriod,typeSeason) %>% summarize(reb = sum(reb), n = sum(n))  %>% mutate(rpg = reb/n) %>% filter(typeSeason == "Regular Season", numberPeriod <= 4) %>% ggplot(aes(numberPeriod,rpg)) + geom_point(size = 5, color = "yellow")+ geom_segment(aes(x=numberPeriod,xend = numberPeriod, y = 0, yend = rpg), color = "grey")+ theme_minimal() + geom_text(aes(label = round(rpg,1)), position = position_dodge(width = 0.9),vjust = -2) + labs(x = "Quarter", y = "Avg Rebounds") + ggtitle("Avg Rebounds Per Quarter Per Game (OT Excluded)")+ scale_y_continuous(breaks = seq(0,50,1))
...
### Regular Season Rebounds per Quarter League Comparison

```{r Quarterly rebounds hist, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}


quarterly_rpg <- quarterly_rebounds  %>% group_by(namePlayer1,idPlayer,numberPeriod,typeSeason) %>% summarize(reb = sum(reb), n = sum(n))  %>% mutate(rpg = reb/n) %>% filter(typeSeason == "Regular Season", numberPeriod <= 4) %>% select(numberPeriod,rpg)

p <- play_play %>% filter(is.na(slugScore), str_detect(descriptionPlayHome,"REBOUND") | str_detect(descriptionPlayVisitor,"REBOUND")) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason) %>% summarize(reb = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA1) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason")) %>% filter(!is.na(slugTeam), typeSeason == "Regular Season", numberPeriod <= 4) %>% mutate(rpg = round(reb/n,1)) %>% ggplot(aes(rpg)) +geom_histogram(fill = "yellow", bins = 25, color = "grey") + facet_wrap(~numberPeriod, scales = 'free') + theme_minimal() + geom_vline(data = quarterly_rpg, aes(xintercept = rpg, color = "blue"),linetype = "dashed") + theme(legend.position = "none") + scale_x_continuous(breaks = seq(0,20,1)) + ggtitle("Avg Rebounds Per Quarter Per Game Distribution (OT Excluded)")

ggplotly(p)

percentile_rebounds_q1 <- play_play %>% filter(is.na(slugScore), str_detect(descriptionPlayHome,"REBOUND") | str_detect(descriptionPlayVisitor,"REBOUND")) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason) %>% summarize(reb = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA1) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason")) %>% filter(!is.na(slugTeam), typeSeason == "Regular Season", numberPeriod <= 4) %>% mutate(rpg = round(reb/n,1)) %>% filter(numberPeriod == 1) %>% mutate(percentile = percent_rank(rpg)*100) %>% filter(idPlayer == player_id[1]) %>% pull(percentile)
...
Leading Indicator: Points {data-navmenu="Leading Indicator Analysis" data-navmenu-icon="fa-chart-area"}
=======================================================================
...
first <- quarterly_points %>% group_by(namePlayer1,idPlayer,numberPeriod,typeSeason) %>% summarize(pts = sum(pts), n = sum(n))  %>% mutate(ppg = pts/n) %>% filter(typeSeason == "Regular Season", numberPeriod == 1) %>% pull(ppg)
zero <- 0
first <- if(length(first) ==0){0}else{first}

points_prob <- seq(from = round(first,0)+1, to = 50, by = 1)

games <- play_play %>% filter(idPlayerNBA1 == player_id[1], !is.na(slugScore)) %>% select(idGame,numberPeriod, descriptionPlayHome,descriptionPlayNeutral,descriptionPlayVisitor, namePlayer1,slugScore,scoreHome,scoreAway,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% mutate(description = ifelse(is.na(descriptionPlayHome),ifelse(is.na(descriptionPlayNeutral),descriptionPlayVisitor,descriptionPlayNeutral),descriptionPlayHome)) %>% mutate(free_throw_flag = str_detect(description,"Free Throw"), three_point_flag = str_detect(description,"3PT")) %>% mutate(two_point_flag = ifelse(free_throw_flag == FALSE & three_point_flag == FALSE, TRUE,FALSE)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% mutate(pts = ifelse(free_throw_flag == TRUE,1,ifelse(two_point_flag== TRUE,2,3)),score_type = ifelse(free_throw_flag == TRUE,"Free Throw",ifelse(two_point_flag== TRUE,"2 pt","3 pt"))) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason,dateGame,idGame) %>% summarize(pts = sum(pts)) %>% filter(numberPeriod == 1, typeSeason == "Regular Season", pts >= first) %>% group_by(idGame,dateGame) %>% summarize(firstq_pts = sum(pts)) %>% mutate(Type = "First Quarter")

games_firstqpts <- games



all <- play_play %>% filter(idPlayerNBA1 == player_id[1], !is.na(slugScore)) %>% select(idGame,numberPeriod, descriptionPlayHome,descriptionPlayNeutral,descriptionPlayVisitor, namePlayer1,slugScore,scoreHome,scoreAway,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% mutate(description = ifelse(is.na(descriptionPlayHome),ifelse(is.na(descriptionPlayNeutral),descriptionPlayVisitor,descriptionPlayNeutral),descriptionPlayHome)) %>% mutate(free_throw_flag = str_detect(description,"Free Throw"), three_point_flag = str_detect(description,"3PT")) %>% mutate(two_point_flag = ifelse(free_throw_flag == FALSE & three_point_flag == FALSE, TRUE,FALSE)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% mutate(pts = ifelse(free_throw_flag == TRUE,1,ifelse(two_point_flag== TRUE,2,3)),score_type = ifelse(free_throw_flag == TRUE,"Free Throw",ifelse(two_point_flag== TRUE,"2 pt","3 pt"))) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason,dateGame,idGame) %>% summarize(pts = sum(pts)) %>% filter(typeSeason == "Regular Season") %>% group_by(idGame,dateGame) %>% summarize(pts = sum(pts)) %>% mutate(Type = "Total")
...
formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this total game point total is scored when ",params$player," scores at least ",round(first,1)," points in 1Q: Regular Season Only"))

```

### First Quarter Leading Indicator - Points Summary

```{r Quarterly points lead summary, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 7, error = TRUE, fig.height = 7}
...
stats <- bind %>% group_by(Type) %>% summarize(mean = mean(pts), SE = sd(pts)) %>% mutate(meanpos = mean + 1 *SE, meanneg = mean - 1*SE)
stats_first <- stats %>% filter(Type == "First Quarter")
stats_total <- stats %>% filter(Type == "Total")

diff <- bind %>% pivot_wider(names_from = Type, values_from = pts) %>% mutate(diff = Total - `First Quarter`)  %>% pivot_longer(!c(dateGame,idGame),names_to = "Type", values_to = "pts") %>% filter(Type == "diff") %>% left_join(b, by = "idGame") %>% mutate(x_pos = pts.y + (pts.x/2))

p <- ggplot(bind) + geom_segment(data = a, aes(x = pts, y = dateGame, yend = b$dateGame, xend = b$pts), color = "grey", size = 3, alpha = 0.5) + geom_point(aes(x = pts, y = dateGame, color = Type), size = 3, show.legend = TRUE) + geom_text(data = diff, aes(x = x_pos, y = dateGame.x, label = paste0("+", pts.x)), fill = "white", color = "#4a4e4d", size = 2.5)  + theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank(),panel.grid.major.x = element_blank(),panel.grid.minor.x = element_blank(),strip.background = element_rect(fill = "white", color = "white")) + scale_color_manual(values = c(primary_color,secondary_color)) + labs(x = "Points", y = "Date") + scale_x_continuous(breaks = seq(0,90,2)) + ggtitle("Point Differential when 1Q pts > Regular Season Avg", subtitle = paste("Differential Range: ",range(diff$pts.x)[1],"-",range(diff$pts.x)[-1],"// MEAN:",round(mean(a$pts),1) )) + theme(legend.position = "top")
...
```{r Quarterly points lead summary less than, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 7, error = TRUE, fig.height = 7}
...
p <- ggplot(bind) + geom_segment(data = a, aes(x = pts, y = dateGame, yend = b$dateGame, xend = b$pts), color = "grey", size = 3, alpha = 0.5) + geom_point(aes(x = pts, y = dateGame, color = Type), size = 3, show.legend = TRUE) + geom_text(data = diff, aes(x = x_pos, y = dateGame.x, label = paste0("+", pts.x)), fill = "white", color = "#4a4e4d", size = 2.5)  + theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank(),panel.grid.major.x = element_blank(),panel.grid.minor.x = element_blank(),strip.background = element_rect(fill = "white", color = "white"))  + scale_color_manual(values = c(primary_color,secondary_color)) + ggtitle("Point Differential when 1Q pts < Regular Season Avg", subtitle = paste("Differential Range: ",range(diff$pts.x)[1],"-",range(diff$pts.x)[-1],"// MEAN:",round(mean(a$pts),1)  )) + labs(x = "Points", y = "Date") + scale_x_continuous(breaks = seq(0,90,2)) + theme(legend.position = "top")
...
Leading Indicator: Assists {data-navmenu="Leading Indicator Analysis"}
=======================================================================
...
quarterly_apg <- play_play %>% filter(idPlayerNBA2 == player_id[1], str_detect(descriptionPlayHome,"AST") | str_detect(descriptionPlayVisitor,"AST")) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,namePlayer2,slugTeamPlayer1,idPlayerNBA1,idPlayerNBA2) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer2,idPlayerNBA2,slugTeam,numberPeriod,typeSeason) %>% summarize(assists = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA2) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason")) %>% group_by(namePlayer2,idPlayer,numberPeriod,typeSeason) %>% summarize(assists = sum(assists), n = sum(n))  %>% mutate(apg = assists/n) %>% filter(typeSeason == "Regular Season", numberPeriod <= 4) %>% select(numberPeriod, apg)

first <- play_play %>% filter(idPlayerNBA2 == player_id[1], str_detect(descriptionPlayHome,"AST") | str_detect(descriptionPlayVisitor,"AST")) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,namePlayer2,slugTeamPlayer1,idPlayerNBA1,idPlayerNBA2) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer2,idPlayerNBA2,slugTeam,numberPeriod,typeSeason) %>% summarize(assists = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA2) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason")) %>% group_by(namePlayer2,idPlayer,numberPeriod,typeSeason) %>% summarize(assists = sum(assists), n = sum(n))  %>% mutate(apg = assists/n) %>% filter(typeSeason == "Regular Season", numberPeriod == 1) %>% pull(apg)
zero <- 0
...
games <- play_play %>% filter(idPlayerNBA2 == player_id[1], str_detect(descriptionPlayHome,"AST") | str_detect(descriptionPlayVisitor,"AST")) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,namePlayer2,slugTeamPlayer1,idPlayerNBA1,idPlayerNBA2) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer2,idPlayerNBA2,slugTeam,numberPeriod,typeSeason,dateGame,idGame) %>% summarize(assists = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA2) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason")) %>% group_by(namePlayer2,idPlayer,numberPeriod,typeSeason,slugTeam,dateGame,idGame) %>% summarize(assists = sum(assists)) %>% filter(numberPeriod == 1, typeSeason == "Regular Season", assists >= first) %>% group_by(idGame,dateGame) %>% summarize(firstq_assists = sum(assists)) %>% mutate(Type = "First Quarter")

games_firstqast <- games

all <- play_play %>% filter(idPlayerNBA2 == player_id[1], str_detect(descriptionPlayHome,"AST") | str_detect(descriptionPlayVisitor,"AST")) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,namePlayer2,slugTeamPlayer1,idPlayerNBA1,idPlayerNBA2) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer2,idPlayerNBA2,slugTeam,numberPeriod,typeSeason,dateGame,idGame) %>% summarize(assists = sum(n)) %>% ungroup() %>% rename(idPlayer = idPlayerNBA2) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason) %>% summarize(n = n()), by = c("idPlayer","typeSeason")) %>% group_by(namePlayer2,idPlayer,numberPeriod,typeSeason,slugTeam,dateGame,idGame) %>% summarize(assists = sum(assists)) %>% filter(typeSeason == "Regular Season") %>% group_by(idGame,dateGame) %>% summarize(assists = sum(assists)) %>% mutate(Type = "Total")
...
formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this game assist total happens when ",params$player," has at least ",round(first,1)," assists in 1Q: Regular Season Only"))

```

### First Quarter Leading Indicator - Assists Summary

```{r Quarterly assists lead summary, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 7, error = TRUE, fig.height = 7}
...
diff <- bind %>% pivot_wider(names_from = Type, values_from = assists) %>% mutate(diff = Total - `First Quarter`)  %>% pivot_longer(!c(dateGame,idGame),names_to = "Type", values_to = "assists") %>% filter(Type == "diff") %>% left_join(b, by = "idGame") %>% mutate(x_pos = assists.y + (assists.x/2))

p <- ggplot(bind) + geom_segment(data = a, aes(x = assists, y = dateGame, yend = b$dateGame, xend = b$assists), color = "grey", size = 3, alpha = 0.5) + geom_point(aes(x = assists, y = dateGame, color = Type), size = 3, show.legend = TRUE) + geom_text(data = diff, aes(x = x_pos, y = dateGame.x, label = paste0("+", assists.x)), fill = "white", color = "#4a4e4d", size = 2.5)  + theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank(),panel.grid.major.x = element_blank(),panel.grid.minor.x = element_blank(),strip.background = element_rect(fill = "white", color = "white"))  + scale_color_manual(values = c(primary_color,secondary_color))  + labs(x = "Assists", y = "Date") + scale_x_continuous(breaks = seq(0,90,1)) + ggtitle("Assist Differential when 1Q Assists > Regular Season Avg", subtitle = paste("Differential Range: ",range(diff$assists.x)[1],"-",range(diff$assists.x)[-1],"// MEAN:",round(mean(a$assists),1)  )) + theme(legend.position = "top")
...
```{r Quarterly assists lead summary less than, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 7, error = TRUE, fig.height = 7}
...
stats <- bind %>% group_by(Type) %>% summarize(mean = mean(assists), SE = sd(pts)) %>% mutate(meanpos = mean + 1 *SE, meanneg = mean - 1*SE)
stats_first <- stats %>% filter(Type == "First Quarter")
stats_total <- stats %>% filter(Type == "Total")

diff <- bind %>% pivot_wider(names_from = Type, values_from = assists) %>% mutate(diff = Total - `First Quarter`)  %>% pivot_longer(!c(dateGame,idGame),names_to = "Type", values_to = "assists") %>% filter(Type == "diff") %>% left_join(b, by = "idGame") %>% mutate(x_pos = assists.y + (assists.x/2))

p <- ggplot(bind) + geom_segment(data = a, aes(x = assists, y = dateGame, yend = b$dateGame, xend = b$assists), color = "grey", size = 3, alpha = 0.5) + geom_point(aes(x = assists, y = dateGame, color = Type), size = 3, show.legend = TRUE) + geom_text(data = diff, aes(x = x_pos, y = dateGame.x, label = paste0("+", assists.x)), fill = "white", color = "#4a4e4d", size = 2.5)  + theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank(),panel.grid.major.x = element_blank(),panel.grid.minor.x = element_blank(),strip.background = element_rect(fill = "white", color = "white"))  + scale_color_manual(values = c(primary_color,secondary_color))  + ggtitle("Assist Differential when 1Q Assists < Regular Season Avg", subtitle = paste("Differential Range: ",range(diff$assists.x)[1],"-",range(diff$assists.x)[-1],"// MEAN:",round(mean(a$assists),1)  )) + labs(x = "Assists", y = "Date") + scale_x_continuous(breaks = seq(0,90,1)) + theme(legend.position = "top")
...
Leading Indicator: Rebounds {data-navmenu="Leading Indicator Analysis"}
=======================================================================
...
first <- quarterly_rebounds %>% group_by(namePlayer1,idPlayer,numberPeriod,typeSeason) %>% summarize(reb = sum(reb), n = sum(n)) %>% mutate(rpg = reb/n) %>% filter(typeSeason == "Regular Season", numberPeriod == 1) %>% pull(rpg) 
...
games <- play_play %>% filter(is.na(slugScore),idPlayerNBA1 == player_id[1], str_detect(descriptionPlayHome,"REBOUND") | str_detect(descriptionPlayVisitor,"REBOUND")) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason,dateGame,idGame) %>% summarize(reb = sum(n)) %>% filter(numberPeriod == 1, typeSeason == "Regular Season", reb >= first) %>% group_by(idGame,dateGame) %>% summarize(firstq_rebounds = sum(reb)) %>% mutate(Type = "First Quarter")

games_firstqreb <- games

all <- play_play %>% filter(is.na(slugScore),idPlayerNBA1 == player_id[1], str_detect(descriptionPlayHome,"REBOUND") | str_detect(descriptionPlayVisitor,"REBOUND")) %>% select(idGame,numberPeriod,descriptionPlayHome,descriptionPlayVisitor, namePlayer1,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason,dateGame,idGame) %>% summarize(reb = sum(n)) %>% filter(typeSeason == "Regular Season") %>% group_by(idGame,dateGame) %>% summarize(reb = sum(reb)) %>% mutate(Type = "Total")
...
formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this game rebound total happens when ",params$player," has at least ",round(first,1)," rebounds in 1Q: Regular Season Only"))

```

### First Quarter Leading Indicator - Rebounds Summary

```{r Quarterly rebounds lead summary, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 7, error = TRUE, fig.height = 7}
...
stats <- bind %>% group_by(Type) %>% summarize(mean = mean(reb), SE = sd(reb)) %>% mutate(meanpos = mean + 1 *SE, meanneg = mean - 1*SE)
stats_first <- stats %>% filter(Type == "First Quarter")
stats_total <- stats %>% filter(Type == "Total")

diff <- bind %>% pivot_wider(names_from = Type, values_from = reb) %>% mutate(diff = Total - `First Quarter`)  %>% pivot_longer(!c(dateGame,idGame),names_to = "Type", values_to = "reb") %>% filter(Type == "diff") %>% left_join(b, by = "idGame") %>% mutate(x_pos = reb.y + (reb.x/2))

p <- ggplot(bind) + geom_segment(data = a, aes(x = reb, y = dateGame, yend = b$dateGame, xend = b$reb), color = "grey", size = 3, alpha = 0.5) + geom_point(aes(x = reb, y = dateGame, color = Type), size = 3, show.legend = TRUE) + geom_text(data = diff, aes(x = x_pos, y = dateGame.x, label = paste0("+", reb.x)), fill = "white", color = "#4a4e4d", size = 2.5)  + theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank(),panel.grid.major.x = element_blank(),panel.grid.minor.x = element_blank(),strip.background = element_rect(fill = "white", color = "white"))  + scale_color_manual(values = c(primary_color,secondary_color))  + labs(x = "Rebounds", y = "Date") + scale_x_continuous(breaks = seq(0,90,1)) + ggtitle("Rebound Differential when 1Q Rebounds > Regular Season Avg", subtitle = paste("Differential Range: ",range(diff$reb.x)[1],"-",range(diff$reb.x)[-1],"// MEAN:",round(mean(a$reb),1)  )) + theme(legend.position = "top")
...
```{r Quarterly rebounds lead summary less than, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 7, error = TRUE, fig.height = 7}
...
stats <- bind %>% group_by(Type) %>% summarize(mean = mean(reb), SE = sd(pts)) %>% mutate(meanpos = mean + 1 *SE, meanneg = mean - 1*SE)
stats_first <- stats %>% filter(Type == "First Quarter")
stats_total <- stats %>% filter(Type == "Total")

diff <- bind %>% pivot_wider(names_from = Type, values_from = reb) %>% mutate(diff = Total - `First Quarter`)  %>% pivot_longer(!c(dateGame,idGame),names_to = "Type", values_to = "reb") %>% filter(Type == "diff") %>% left_join(b, by = "idGame") %>% mutate(x_pos = reb.y + (reb.x/2))

p <- ggplot(bind) + geom_segment(data = a, aes(x = reb, y = dateGame, yend = b$dateGame, xend = b$reb), color = "grey", size = 3, alpha = 0.5) + geom_point(aes(x = reb, y = dateGame, color = Type), size = 3, show.legend = TRUE) + geom_text(data = diff, aes(x = x_pos, y = dateGame.x, label = paste0("+", reb.x)), fill = "white", color = "#4a4e4d", size = 2.5)  + theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank(),panel.grid.major.x = element_blank(),panel.grid.minor.x = element_blank(),strip.background = element_rect(fill = "white", color = "white")) + scale_color_manual(values = c(primary_color,secondary_color))  + ggtitle("Rebound Differential when 1Q Rebounds < Regular Season Avg", subtitle = paste("Differential Range: ",range(diff$reb.x)[1],"-",range(diff$reb.x)[-1],"// MEAN:",round(mean(a$reb),1)  )) + labs(x = "Rebounds", y = "Date") + scale_x_continuous(breaks = seq(0,90,1)) + theme(legend.position = "top")
...
Leading Indicator: Three Pointers {data-navmenu="Leading Indicator Analysis" data-navmenu-icon="fa-chart-area"}
=======================================================================
...
quarterly_threes <-play_play %>% filter(idPlayerNBA1 == player_id[1], !is.na(slugScore)) %>% select(idGame,numberPeriod, descriptionPlayHome,descriptionPlayNeutral,descriptionPlayVisitor, namePlayer1,slugScore,scoreHome,scoreAway,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% mutate(description = ifelse(is.na(descriptionPlayHome),ifelse(is.na(descriptionPlayNeutral),descriptionPlayVisitor,descriptionPlayNeutral),descriptionPlayHome)) %>% mutate(free_throw_flag = str_detect(description,"Free Throw"), three_point_flag = str_detect(description,"3PT")) %>% mutate(two_point_flag = ifelse(free_throw_flag == FALSE & three_point_flag == FALSE, TRUE,FALSE)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% mutate(pts = ifelse(free_throw_flag == TRUE,1,ifelse(two_point_flag== TRUE,2,3)),score_type = ifelse(free_throw_flag == TRUE,"Free Throw",ifelse(two_point_flag== TRUE,"2 pt","3 pt"))) %>% filter(score_type == "3 pt") %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason) %>% summarize(fg3m = n()) %>% ungroup() %>% rename(idPlayer = idPlayerNBA1) %>% left_join(playerdata  %>% group_by(idPlayer,typeSeason,slugTeam) %>% summarize(n = n()), by = c("idPlayer","typeSeason","slugTeam"))

quarterly_threes <- replace(quarterly_threes,is.na(quarterly_threes),0)



quarterly_3pg <- quarterly_threes %>% group_by(namePlayer1,idPlayer,numberPeriod,typeSeason) %>% summarize(fg3m = sum(fg3m), n = sum(n))  %>% mutate(threepg = fg3m/n) %>% filter(typeSeason == "Regular Season", numberPeriod <= 4) %>% select(numberPeriod,threepg)

first <- quarterly_threes %>% group_by(namePlayer1,idPlayer,numberPeriod,typeSeason) %>% summarize(fg3m = sum(fg3m), n = sum(n))  %>% mutate(threepg = fg3m/n) %>% filter(typeSeason == "Regular Season", numberPeriod == 1) %>% pull(threepg)
zero <- 0
first <- if(length(first) ==0){0}else{first}

points_prob <- seq(from = round(first,0)+1, to = 8, by = 1)

games <- play_play %>% filter(idPlayerNBA1 == player_id[1], !is.na(slugScore)) %>% select(idGame,numberPeriod, descriptionPlayHome,descriptionPlayNeutral,descriptionPlayVisitor, namePlayer1,slugScore,scoreHome,scoreAway,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% mutate(description = ifelse(is.na(descriptionPlayHome),ifelse(is.na(descriptionPlayNeutral),descriptionPlayVisitor,descriptionPlayNeutral),descriptionPlayHome)) %>% mutate(free_throw_flag = str_detect(description,"Free Throw"), three_point_flag = str_detect(description,"3PT")) %>% mutate(two_point_flag = ifelse(free_throw_flag == FALSE & three_point_flag == FALSE, TRUE,FALSE)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% mutate(pts = ifelse(free_throw_flag == TRUE,1,ifelse(two_point_flag== TRUE,2,3)),score_type = ifelse(free_throw_flag == TRUE,"Free Throw",ifelse(two_point_flag== TRUE,"2 pt","3 pt"))) %>% filter(score_type == "3 pt") %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason,dateGame,idGame) %>% summarize(fg3m = n()) %>% filter(numberPeriod == 1, typeSeason == "Regular Season", fg3m >= first) %>% group_by(idGame,dateGame) %>% summarize(firstq_threes = sum(fg3m)) %>% mutate(Type = "First Quarter")



all <- play_play %>% filter(idPlayerNBA1 == player_id[1], !is.na(slugScore)) %>% select(idGame,numberPeriod, descriptionPlayHome,descriptionPlayNeutral,descriptionPlayVisitor, namePlayer1,slugScore,scoreHome,scoreAway,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% mutate(description = ifelse(is.na(descriptionPlayHome),ifelse(is.na(descriptionPlayNeutral),descriptionPlayVisitor,descriptionPlayNeutral),descriptionPlayHome)) %>% mutate(free_throw_flag = str_detect(description,"Free Throw"), three_point_flag = str_detect(description,"3PT")) %>% mutate(two_point_flag = ifelse(free_throw_flag == FALSE & three_point_flag == FALSE, TRUE,FALSE)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% mutate(pts = ifelse(free_throw_flag == TRUE,1,ifelse(two_point_flag== TRUE,2,3)),score_type = ifelse(free_throw_flag == TRUE,"Free Throw",ifelse(two_point_flag== TRUE,"2 pt","3 pt"))) %>% filter(score_type == "3 pt") %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason,dateGame,idGame) %>% summarize(fg3m = n()) %>% filter(typeSeason == "Regular Season") %>% group_by(idGame,dateGame) %>% summarize(fg3m = sum(fg3m)) %>% mutate(Type = "Total")
...
formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this threes made total is scored when ",params$player," makes at least ",round(first,1)," threes in 1Q: Regular Season Only"))

```

### First Quarter Leading Indicator - Three Pointers Summary

```{r Quarterly three points lead summary, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 7, error = TRUE, fig.height = 7}

a <- bind %>% filter(Type == "Total")
a_sorted <- a %>% arrange(desc(dateGame))
b <- bind %>% filter(Type == "First Quarter")

a_fg3m <- round(mean(a$fg3m),1)

stats <- bind %>% group_by(Type) %>% summarize(mean = mean(fg3m), SE = sd(fg3m)) %>% mutate(meanpos = mean + 1 *SE, meanneg = mean - 1*SE)
stats_first <- stats %>% filter(Type == "First Quarter")
stats_total <- stats %>% filter(Type == "Total")

diff <- bind %>% pivot_wider(names_from = Type, values_from = fg3m) %>% mutate(diff = Total - `First Quarter`)  %>% pivot_longer(!c(dateGame,idGame),names_to = "Type", values_to = "fg3m") %>% filter(Type == "diff") %>% left_join(b, by = "idGame") %>% mutate(x_pos = fg3m.y + (fg3m.x/2))

p <- ggplot(bind) + geom_segment(data = a, aes(x = fg3m, y = dateGame, yend = b$dateGame, xend = b$fg3m), color = "grey", size = 3, alpha = 0.5) + geom_point(aes(x = fg3m, y = dateGame, color = Type), size = 3, show.legend = TRUE) + geom_text(data = diff, aes(x = x_pos, y = dateGame.x, label = paste0("+", fg3m.x)), fill = "white", color = "#4a4e4d", size = 2.5)  + theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank(),panel.grid.major.x = element_blank(),panel.grid.minor.x = element_blank(),strip.background = element_rect(fill = "white", color = "white")) + scale_color_manual(values = c(primary_color,secondary_color)) + labs(x = "FG3M", y = "Date") + scale_x_continuous(breaks = seq(0,90,2)) + ggtitle("FG3M Differential when 1Q FG3M > Regular Season Avg", subtitle = paste("Differential Range: ",range(diff$fg3m.x)[1],"-",range(diff$fg3m.x)[-1],"// MEAN:",round(mean(a$fg3m),1) )) + theme(legend.position = "top")
...
```{r Quarterly three points lead summary less than, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 7, error = TRUE, fig.height = 7}

games <- play_play %>% filter(idPlayerNBA1 == player_id[1], !is.na(slugScore)) %>% select(idGame,numberPeriod, descriptionPlayHome,descriptionPlayNeutral,descriptionPlayVisitor, namePlayer1,slugScore,scoreHome,scoreAway,slugTeamPlayer1,idPlayerNBA1) %>% rename(slugTeam = slugTeamPlayer1) %>% mutate(description = ifelse(is.na(descriptionPlayHome),ifelse(is.na(descriptionPlayNeutral),descriptionPlayVisitor,descriptionPlayNeutral),descriptionPlayHome)) %>% mutate(free_throw_flag = str_detect(description,"Free Throw"), three_point_flag = str_detect(description,"3PT")) %>% mutate(two_point_flag = ifelse(free_throw_flag == FALSE & three_point_flag == FALSE, TRUE,FALSE)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason,locationGame,slugTeam) %>% summarize(n = n()), by = c("idGame","slugTeam")) %>% mutate(pts = ifelse(free_throw_flag == TRUE,1,ifelse(two_point_flag== TRUE,2,3)),score_type = ifelse(free_throw_flag == TRUE,"Free Throw",ifelse(two_point_flag== TRUE,"2 pt","3 pt"))) %>% filter(score_type == "3 pt") %>% group_by(namePlayer1,idPlayerNBA1,slugTeam,numberPeriod,typeSeason,dateGame,idGame) %>% summarize(fg3m = n()) %>% filter(numberPeriod == 1, typeSeason == "Regular Season", fg3m < first) %>% group_by(idGame,dateGame) %>% summarize(firstq_fg3m = sum(fg3m)) %>% mutate(Type = "First Quarter")

error <- all %>% mutate(fg3m = fg3m*0, Type = "First Quarter") %>% rename(firstq_fg3m = fg3m)
games <- if(nrow(games)==0){error}else{games}


bind <- bind_rows(games %>% rename(fg3m = firstq_fg3m),all %>% semi_join(games, by = "idGame"))
...
p <- ggplot(bind) + geom_segment(data = a, aes(x = fg3m, y = dateGame, yend = b$dateGame, xend = b$fg3m), color = "grey", size = 3, alpha = 0.5) + geom_point(aes(x = fg3m, y = dateGame, color = Type), size = 3, show.legend = TRUE) + geom_text(data = diff, aes(x = x_pos, y = dateGame.x, label = paste0("+", fg3m.x)), fill = "white", color = "#4a4e4d", size = 2.5)  + theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank(),panel.grid.major.x = element_blank(),panel.grid.minor.x = element_blank(),strip.background = element_rect(fill = "white", color = "white"))  + scale_color_manual(values = c(primary_color,secondary_color)) + ggtitle("FG3M Differential when 1Q FG3M < Regular Season Avg", subtitle = paste("Differential Range: ",range(diff$fg3m.x)[1],"-",range(diff$fg3m.x)[-1],"// MEAN:",round(mean(a$fg3m),1)  )) + labs(x = "FG3M", y = "Date") + scale_x_continuous(breaks = seq(0,90,2)) + theme(legend.position = "top")
...
Leading Indicator: Pts+Reb+Ast {data-navmenu="Leading Indicator Analysis" data-navmenu-icon="fa-chart-area"}
=======================================================================
...
quarterly_ptsrebast <- quarterly_points %>% left_join(quarterly_rebounds, by = c("idPlayer","typeSeason","slugTeam","numberPeriod")) %>% left_join(quarterly_assists, by = c("idPlayer","typeSeason","slugTeam","numberPeriod")) %>% rename(namePlayer1 = namePlayer1.x) %>% select(namePlayer1,idPlayer,slugTeam,numberPeriod,typeSeason,pts,reb,assists,n.x.x)

quarterly_ptsrebast <- replace(quarterly_ptsrebast,is.na(quarterly_ptsrebast),0)



quarterly_ptsrebastpg <- quarterly_ptsrebast %>% group_by(namePlayer1,idPlayer,numberPeriod,typeSeason) %>% summarize(pts = sum(pts), n = sum(n.x.x), ast = sum(assists), reb = sum(reb))  %>% mutate(ptsrebastpg = (pts+ast+reb)/n) %>% filter(typeSeason == "Regular Season", numberPeriod <= 4) %>% select(numberPeriod,ptsrebastpg)

first <- quarterly_ptsrebast %>% group_by(namePlayer1,idPlayer,numberPeriod,typeSeason) %>% summarize(pts = sum(pts), n = sum(n.x.x), ast = sum(assists), reb = sum(reb))  %>% mutate(ptsrebastpg = (pts+ast+reb)/n) %>% filter(typeSeason == "Regular Season", numberPeriod == 1) %>% pull(ptsrebastpg)
zero <- 0
first <- if(length(first) ==0){0}else{first}
...
all <- all_firstqpts %>% full_join(all_firstqast, by = c("idGame","dateGame","Type")) %>% full_join(all_firstqreb, by = c("idGame","dateGame","Type")) %>% mutate(ptsrebast = pts + assists + reb) %>% select(idGame,dateGame,ptsrebast,Type)
...
formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this Pts+Reb+Ast total is attained when ",params$player," has at least ",round(first,1)," Pts+Reb+Ast in 1Q: Regular Season Only"))

```

### First Quarter Leading Indicator - Pts+Reb+Ast Summary

```{r Quarterly ptsrebast points lead summary, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 7, error = TRUE, fig.height = 7}

a <- bind %>% filter(Type == "Total") %>% arrange(desc(dateGame))
a_sorted <- a %>% arrange(desc(dateGame))
b <- bind %>% filter(Type == "First Quarter") %>% arrange(desc(dateGame))

a_ptsrebast <- round(mean(a$ptsrebast),1)

stats <- bind %>% group_by(Type) %>% summarize(mean = mean(ptsrebast), SE = sd(ptsrebast)) %>% mutate(meanpos = mean + 1 *SE, meanneg = mean - 1*SE)
stats_first <- stats %>% filter(Type == "First Quarter")
stats_total <- stats %>% filter(Type == "Total")

diff <- bind %>% pivot_wider(names_from = Type, values_from = ptsrebast) %>% mutate(diff = Total - `First Quarter`)  %>% pivot_longer(!c(dateGame,idGame),names_to = "Type", values_to = "ptsrebast") %>% filter(Type == "diff") %>% left_join(b, by = "idGame") %>% mutate(x_pos = ptsrebast.y + (ptsrebast.x/2))

p <- ggplot(bind) + geom_segment(data = a, aes(x = ptsrebast, y = dateGame, yend = b$dateGame, xend = b$ptsrebast), color = "grey", size = 3, alpha = 0.5) + geom_point(aes(x = ptsrebast, y = dateGame, color = Type), size = 3, show.legend = TRUE) + geom_text(data = diff, aes(x = x_pos, y = dateGame.x, label = paste0("+", ptsrebast.x)), fill = "white", color = "#4a4e4d", size = 2.5)  + theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank(),panel.grid.major.x = element_blank(),panel.grid.minor.x = element_blank(),strip.background = element_rect(fill = "white", color = "white")) + scale_color_manual(values = c(primary_color,secondary_color)) + labs(x = "Pts+Reb+Ast", y = "Date") + scale_x_continuous(breaks = seq(0,90,2)) + ggtitle("Differential when 1Q Pts+Reb+Ast > Regular Season Avg", subtitle = paste("Differential Range: ",range(diff$ptsrebast.x)[1],"-",range(diff$ptsrebast.x)[-1],"// MEAN:",round(mean(a$ptsrebast),1) )) + theme(legend.position = "top")
...
```{r Quarterly ptsrebast lead summary less than, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 7, error = TRUE, fig.height = 7}

games <- games_firstqpts_less %>% full_join(games_firstqast_less, by = c("idGame","dateGame","Type")) %>% full_join(games_firstqreb_less, by = c("idGame","dateGame","Type"))
...
a_less <- round(mean(a$ptsrebast),1)
...
p <- ggplot(bind) + geom_segment(data = a, aes(x = ptsrebast, y = dateGame, yend = b$dateGame, xend = b$ptsrebast), color = "grey", size = 3, alpha = 0.5) + geom_point(aes(x = ptsrebast, y = dateGame, color = Type), size = 3, show.legend = TRUE) + geom_text(data = diff, aes(x = x_pos, y = dateGame.x, label = paste0("+", ptsrebast.x)), fill = "white", color = "#4a4e4d", size = 2.5)  + theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank(),panel.grid.major.x = element_blank(),panel.grid.minor.x = element_blank(),strip.background = element_rect(fill = "white", color = "white"))  + scale_color_manual(values = c(primary_color,secondary_color)) + ggtitle("Differential when 1Q Pts+Reb+Ast < Regular Season Avg", subtitle = paste("Differential Range: ",range(diff$ptsrebast.x)[1],"-",range(diff$ptsrebast.x)[-1],"// MEAN:",round(mean(a$ptsrebast),1)  )) + labs(x = "Pts+Reb+Ast", y = "Date") + scale_x_continuous(breaks = seq(0,90,2)) + theme(legend.position = "top")
...
Regular Season {data-navmenu="Shot Charts" data-navmenu-icon="fa-chart-simple"}
=======================================================================

Row
-----------------------------------------------------------------------

### Shot Distribution FG% Summary - Regular Season {data-width=1300}

```{r  season shot chart details, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', fig.height = 9, fig.width = 9}
...
# Grab team shot data
team <- teams_shots(team_ids = team_id, seasons = 2024, season_types = "Regular Season", return_message = FALSE)
...
shot_summary <- player %>% select(zoneBasic,nameZone,slugZone,distanceShot,isShotAttempted,isShotMade,typeAction,typeShot,namePlayer) %>% group_by(zoneBasic,namePlayer) %>% summarize(attempts = n(),makes = sum(isShotMade,na.rm = TRUE)) %>% mutate(misses = attempts - makes,fg = makes/attempts) 
...
```{r  season shot chart, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', fig.height = 9}
...
plot_court = function(court_theme = court_themes$custom, use_short_three = FALSE) {
  if (use_short_three) {
...
#grab <- nba_teams() %>% filter(isNonNBATeam == 0)
...
#player <- player %>% filter(dateGame == 20201222)


# NBA Chart

p1 <- plot_court(court_themes$custom2, use_short_three = F) +
  ...
  scale_fill_manual(values = c('green4','red3'), aesthetics = "fill", breaks=c("TRUE", "FALSE"), labels=c("Made", "Missed")) +
  ...
  theme(plot.title = element_text(hjust = .5, size = 22,  face = "bold", vjust = -4),
        ...
        plot.caption = element_text(hjust = .5, size = 6,  face = "bold", colour = "white", vjust = 8)) +
  ggtitle(label = paste(params$player,"Shot Chart"),
          subtitle = "Regular Season Excludes OT") +
  labs(caption = "")
...
Regular Season Q1 {data-navmenu="Shot Charts"}
=======================================================================

Row
-----------------------------------------------------------------------

### Shot Distribution FG% Summary - Regular Season Q1 {data-width=1300}

```{r  season shot chart details q1, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', fig.height = 9, fig.width = 9}
...
# Filter shot data for player & clean data to fit court dimensions
player <- team %>% filter(idPlayer == player_id[1], numberPeriod == 1) %>% 
  mutate(x = as.numeric(as.character(locationX)) / 10, y = as.numeric(as.character(locationY)) / 10 + hoop_center_y)
...
```{r Q1 shot chart, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', fig.height = 9}
...
p1 <- plot_court(court_themes$custom2, use_short_three = F) +
  ...
  theme(plot.title = element_text(hjust = .5, size = 22,  face = "bold", vjust = -4),
        ...
        plot.caption = element_text(hjust = .5, size = 6,  face = "bold", colour = "black", vjust = 8)) +
  ggtitle(label = paste(params$player,"Q1 Shot Chart"),
...
Regular Season Q2 {data-navmenu="Shot Charts"}
=======================================================================

Row
-----------------------------------------------------------------------

### Shot Distribution FG% Summary - Regular Season Q2 {data-width=1300}

```{r  season shot chart details q2, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', fig.height = 9, fig.width = 9}
...
# Filter shot data for player & clean data to fit court dimensions
player <- team %>% filter(idPlayer == player_id[1], numberPeriod == 2) %>% 
  mutate(x = as.numeric(as.character(locationX)) / 10, y = as.numeric(as.character(locationY)) / 10 + hoop_center_y)
...
```{r Q2 shot chart, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', fig.height = 9}
...
p1 <- plot_court(court_themes$custom2, use_short_three = F) +
  ...
  theme(plot.title = element_text(hjust = .5, size = 22,  face = "bold", vjust = -4),
        ...
        legend.text = element_text(hjust = .5, size = 10,  face = "bold", colour = "black"),
        plot.caption = element_text(hjust = .5, size = 6,  face = "bold", colour = "black", vjust = 8)) +
  ggtitle(label = paste(params$player,"Q2 Shot Chart"),
          subtitle = "Regular Season") +
  labs(caption = "")
...
Regular Season Q3 {data-navmenu="Shot Charts"}
=======================================================================

Row
-----------------------------------------------------------------------

### Shot Distribution FG% Summary - Regular Season Q3 {data-width=1300}

```{r  season shot chart details q3, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', fig.height = 9, fig.width = 9}
...
# Filter shot data for player & clean data to fit court dimensions
player <- team %>% filter(idPlayer == player_id[1], numberPeriod == 3) %>% 
  mutate(x = as.numeric(as.character(locationX)) / 10, y = as.numeric(as.character(locationY)) / 10 + hoop_center_y)
...
radarchart(shot_summary_2, axis = 1, pcol = rgb(0.2,0.5,0.5,0.9), pfcol = rgb(0,0.5,0.5,0.5) , plwd=4,cglcol="grey", cglty=1, axislabcol="grey", cglwd=1.2,vlcex=1, title = paste(params$player,"FG% Distribution Regular Season Q3 Excludes OT"))
...
```{r Q3 shot chart, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', fig.height = 9}
...
p1 <- plot_court(court_themes$custom2, use_short_three = F) +
  ...
  theme(plot.title = element_text(hjust = .5, size = 22,  face = "bold", vjust = -4),
        ...
        legend.text = element_text(hjust = .5, size = 10,  face = "bold", colour = "black"),
        plot.caption = element_text(hjust = .5, size = 6,  face = "bold", colour = "black", vjust = 8)) +
  ggtitle(label = paste(params$player,"Q3 Shot Chart"),
          subtitle = "Regular Season") +
  labs(caption = "")
...
Regular Season Q4 {data-navmenu="Shot Charts"}
=======================================================================

Row
-----------------------------------------------------------------------

### Shot Distribution FG% Summary - Regular Season Q4 {data-width=1300}

```{r  season shot chart details q4, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', fig.height = 9, fig.width = 9}
...
radarchart(shot_summary_2, axis = 1, pcol = rgb(0.2,0.5,0.5,0.9), pfcol = rgb(0,0.5,0.5,0.5) , plwd=4,cglcol="grey", cglty=1, axislabcol="grey", cglwd=1.2,vlcex=1, title = paste(params$player,"FG% Distribution Regular Season Q4 Excludes OT"))
...
```{r Q4 shot chart, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', fig.height = 9}
...
p1 <- plot_court(court_themes$custom2, use_short_three = F) +
  ...
  theme(plot.title = element_text(hjust = .5, size = 22,  face = "bold", vjust = -4),
        ...
        legend.text = element_text(hjust = .5, size = 10,  face = "bold", colour = "black"),
        plot.caption = element_text(hjust = .5, size = 6,  face = "bold", colour = "black", vjust = 8)) +
  ggtitle(label = paste(params$player,"Q4 Shot Chart"),
          subtitle = "Regular Season") +
  labs(caption = "")
...
Shot Frequency Heat Maps {data-navmenu="Shot Charts"}
=======================================================================

Column {.tabset .tabset-fade .tabset-pills}
-----------------------------------------------------------------------

### Regular Season Shot Frequency Heatmap

```{r  season shot chart Test, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center'}
...
p1 <- plot_court(court_themes$custom, use_short_three = F) +
  ...
  scale_x_continuous(limits = c(-27.5, 27.5)) +
  scale_y_continuous(limits = c(0, 45)) +
  theme(plot.title = element_text(hjust = .5, size = 18,  face = "bold", vjust = -4),
        plot.subtitle = element_text(hjust = .5, size = 10,  face = "bold", vjust = -8),
        legend.position = "none",
        legend.direction = "horizontal",
        legend.title = element_blank(),
        legend.text = element_text(hjust = .5, size = 8,  face = "bold", colour = "black"),
        plot.caption = element_text(hjust = .5, size = 6,  face = "bold", colour = "black", vjust = 8)) +
  ggtitle(label = paste(params$player,"Shot Frequency Heat Map"),
          subtitle = "Regular Season Excludes OT") +
  labs(caption = "")
...
ggdraw(p1) + theme(plot.background = element_rect(fill="white", color = NA)) + draw_image(logo, scale = .1, x=0.35, y = 0.25) + draw_image(pic, scale = .2, x=0.4, y = 0.2) 
...
### Regular Season Q1 Shot Frequency Heatmap

```{r  season shot chart Q1, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center'}
...
### Regular Season Q2 Shot Frequency Heatmap

```{r  season shot chart Q2, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center'}
...
### Regular Season Q3 Shot Frequency Heatmap

```{r  season shot chart Q3, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center'}
...
### Regular Season Q4 Shot Frequency Heatmap

```{r  season shot chart Q4, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center'}
...
```{r  season shot chart Test Test, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', eval = FALSE}
...
palette <- paletteer::paletteer_d("RColorBrewer::GnBu", direction = -1)
...
p1 <- plot_court(court_themes$dark, use_short_three = F) +
  ...
  theme(plot.title = element_text(hjust = .5, size = 22,  face = "bold", vjust = -4),
        ...
        legend.text = element_text(hjust = .5, size = 10,  face = "bold", colour = "black"),
        plot.caption = element_text(hjust = .5, size = 6,  face = "bold", colour = "black", vjust = 8)) +
  ggtitle(label = paste(params$player,"Frequency Heat Map"),
          subtitle = "Regular Season Excludes OT") +
  labs(caption = "")



ggdraw(p1) + theme(plot.background = element_rect(fill="black", color = NA)) + draw_image(logo, scale = .1, x=0.25, y = 0.25) + draw_image(pic, scale = .2, x=0.3, y = 0.2) 



```

```{r 1q Attempts vs Points, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", eval = FALSE}


attempt%>% ggplot(aes(x = dateGame)) + geom_line(aes(y = pts, group = 1), color = "blue") + geom_line(aes(y = attempts, group = 1), color = "red") + theme_minimal() + labs(x = "Date") + scale_y_continuous(breaks = seq(0,60,1)) + ggtitle(toupper(paste(params$player,"1Q Points vs Attempts ","(Blue: Points","Red: Attempts)")))
```


Opponent Summary {data-navmenu="Data by Opponent" data-navmenu-icon="fa-chart-gantt"}
=======================================================================
...
formattable(season_opponent_table, align = "c", list(min = color_tile("white","gold"),PPG = color_tile("white","gold"),RPG = color_tile("white","gold"),APG = color_tile("white","gold"),SPG = color_tile("white","gold"),BPG = color_tile("white","gold"),TOV = color_tile("white","gold"),FG3M = color_tile("white","gold"),`FG%` = color_tile("white","gold"),`3FG%` = color_tile("white","gold"), `Pts+Reb+Ast` = color_tile("white","gold"), `Pts+Ast` = color_tile("white","gold"), `Pts+Reb` = color_tile("white","gold"), `Reb+Ast` = color_tile("white","gold"), `First Scorer Count` = color_tile("white","gold"), `Q1 PPG` = color_tile("white","gold"), `Q1 APG` = color_tile("white","gold"), `Q1 RPG` = color_tile("white","gold")), caption = "Against Opponents: Regular Season")
...
season_opponent_table <- player_data %>% left_join(play_play_rebounds %>% filter(numberPeriod == 1) %>% group_by(idGame) %>% summarize(firstqrebounds = n()), by = "idGame") %>% mutate(firstqrebounds = replace_na(firstqrebounds,0)) %>% left_join(play_play_assists %>% filter(numberPeriod == 1) %>% group_by(idGame) %>% summarize(firstqassists = n()), by = "idGame") %>% mutate(firstqassists = replace_na(firstqassists,0)) %>% left_join(play_play_makes %>% filter(numberPeriod == 1) %>% group_by(idGame,namePlayer1,slugScore,scoreHome,scoreAway,score_type) %>% summarize(n = n(),firstqpoints = sum(pts)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason) %>% summarize(n = n()), by = "idGame") %>% group_by(idGame) %>% summarize(firstqpoints = sum(firstqpoints)), by = "idGame") %>% mutate(firstqpoints = replace_na(firstqpoints,0)) %>% left_join(play_play_makes %>% group_by(idGame,namePlayer1,slugScore,scoreHome,scoreAway,score_type) %>% summarize(n = n()) %>% mutate(first = ifelse((scoreHome >0 & scoreAway == 0) | (scoreAway > 0 & scoreHome == 0),1,0))%>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason) %>% summarize(n = n()), by = "idGame") %>% group_by(idGame,first) %>% summarize(n = n()) %>% filter(first == 1), by = "idGame") %>% mutate(first = replace_na((first),0)) %>% filter(typeSeason == "Regular Season", locationGame == "H") %>% group_by(slugOpponent) %>% summarize(n = n(), total_pts = sum(pts), total_reb = sum(treb), total_ast = sum(ast), total_stl = sum(stl), total_blk = sum(blk), total_fgm = sum(fgm), total_fga = sum(fga), total_fg3m = sum(fg3m), total_fg3a = sum(fg3a), total_minutes = sum(minutes), total_tov = sum(tov), total_first = sum(first), totalfqpts = sum(firstqpoints), totalfqast = sum(firstqassists), totalfqtreb = sum(firstqrebounds))  %>% mutate(ppg = round(total_pts/n,1), rpg = round(total_reb/n,1), apg = round(total_ast/n,1), spg = round(total_stl/n,1), bpg = round(total_blk/n,1), fg_percentage = round((total_fgm/total_fga)*100,1), fg3_percentage = round((total_fg3m/total_fg3a)*100,1), min = round(total_minutes/n,1),threepg = round(total_fg3m/n,1), tovpg = round(total_tov/n,1), pts_reb_ast = round((total_reb+total_pts+total_ast)/n,1), pts_ast = round((total_pts+total_ast)/n,1), pts_reb = round((total_pts+total_reb)/n,1), ast_reb = round((total_ast+total_reb)/n,1),firstqppg = round(totalfqpts/n,1), firstqapg = round(totalfqast/n,1), firstqrpg = round(totalfqtreb/n,1)) %>% select(slugOpponent,n,min,ppg,rpg,apg,spg,bpg,threepg,fg_percentage,fg3_percentage,tovpg,pts_reb_ast,pts_ast,pts_reb,ast_reb, total_first,firstqppg,firstqapg,firstqrpg) %>%
  rename(GP = n,
        Opponent = slugOpponent, 
        Min = min,
         PPG = ppg,
...
Geographic Analysis {data-navmenu="Data by Opponent" .hidden}
=======================================================================
...
```{r map, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, error = TRUE}
...
```{r Season Opponents Away Summary, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE}
...
Defensive Rating Analysis {data-navmenu="Data by Opponent"}
=======================================================================
...
```{r defensive rating, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE, fig.width = 9, results = 'hide'}
...
rating <- suppressWarnings(suppressPackageStartupMessages(suppressMessages(unnest(bref_teams_stats(seasons = 2024, return_message = FALSE)))))
...
```{r defensive rating ast, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE, fig.width = 9, results = 'hide'}
...
```{r defensive rating treb, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE, fig.width = 9, results = 'hide'}
...
```{r defensive rating fg3m, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE, fig.width = 9, results = 'hide'}
...
```{r defensive rating tov, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE, fig.width = 9, results = 'hide'}
...
```{r defensive rating pt+reb+ast, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE, fig.width = 9, results = 'hide'}
...
```{r defensive rating pt+ast, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE, fig.width = 9, results = 'hide'}
...
```{r defensive rating pt+reb, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE, fig.width = 9, results = 'hide'}
...
```{r defensive rating reb_ast, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE, fig.width = 9, results = 'hide'}
...
Defensive Rating Analysis: Q1 PPG {data-navmenu="Data by Opponent" .hidden}
=======================================================================
...
```{r defensive rating q1 ppg, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE, fig.width = 9, results = 'hide'}
...
```{r defensive rating summary q1 ppg, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE}
...
Defensive Rating Analysis: Q1 APG {data-navmenu="Data by Opponent" .hidden}
=======================================================================
...
```{r defensive rating q1 apg, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE, fig.width = 9, results = 'hide'}
...
```{r defensive rating summary q1 apg, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE}
...
Defensive Rating Analysis: Q1 RPG {data-navmenu="Data by Opponent" .hidden}
=======================================================================
...
```{r defensive rating q1 rpg, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE, fig.width = 9, results = 'hide'}
...
```{r defensive rating summary q1 rpg, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE}
...
```{r Exploratory PPG b2b, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}
...
```{r b2b pts boxplot, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}
...
```{r Exploratory APG b2b, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}
...
```{r b2b ast boxplot, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}
...
```{r Exploratory RPG b2b, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}
...
```{r b2b reb boxplot, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}
...
```{r Exploratory Points in the Paint, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", error = TRUE}
...
```{r Exploratory Fast Break Points, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", error = TRUE}
...
p <- ggdraw() + draw_image(logo,scale = .6, x = -0.3, y = 0.2)+ draw_image(photo) + theme(plot.background = element_rect(fill = "#F2DFDD",color = "#F2DFDD"))
...
<center> <p>Teams Played: `r team_name` </p></center>



```{r summary sidebar, echo = FALSE, warning=FALSE, message = FALSE, error = TRUE,fig.align='left', eval = FALSE}

df3 <- df2 %>% select(Type,GP,Min,PPG)

df3 <- df3[1:5,]

formattable(df3, align = "c")
...
The Low Down {data-navmenu-icon="fa-signal"}
=======================================================================
<center> <h1>The Low Down</center> </h1>

* In the past 10 games played, `r params$player` has averaged **`r season_last10$ppg` points per game**, **`r season_last10$apg` assists per game**, **`r season_last10$rpg` rebounds per game**
* Against `r params$opponent` in the regular season, `r params$player` has averaged **`r season_opponent$ppg` points per game**, **`r season_opponent$apg` assists per game**, **`r season_opponent$rpg` rebounds per game**
* In the regular season, `r params$player` averaged **`r season$pts_reb_ast`** combined points, assists, and rebounds. In the past 5 games, the average combined points, assists, and rebounds is **`r season_last5$pts_reb_ast`: `r paste0(round(((season_last5$pts_reb_ast / season$pts_reb_ast) - 1)* 100,0),"%")` change**
* In the regular season 1st Quarter, `r params$player` averaged **`r season$firstqppg`** points per game, **`r season$firstqapg`** assists per game, **`r season$firstqrpg`** rebounds per game
    * When `r params$player` reaches his Q1 PPG avg of `r season$firstqppg`, his total PPG avg is **`r a_pts`**. When it is not reached, his total PPG avg is **`r a_less`**, a difference of **`r a_pts - a_less`** points. His 1st Quarter PPG average is in the top **`r round(percentile_points_q1,0)` percentile** of the league
    * When `r params$player` reaches his Q1 APG avg of `r season$firstqapg`, his total APG avg is **`r a_ast`**. When it is not reached, his total APG avg is **`r a_ast_less`**, a difference of **`r a_ast - a_ast_less`** assists. His 1st Quarter APG average is in the top **`r round(percentile_assists_q1,0)` percentile** of the league
    * When `r params$player` reaches his Q1 RPG avg of `r season$firstqrpg`, his total APG avg is **`r a_reb`**. When it is not reached, his total APG avg is **`r a_reb_less`**, a difference of **`r a_reb - a_reb_less`** assists. His 1st Quarter RPG average is in the top **`r round(percentile_rebounds_q1,0)` percentile** of the league
* In the regular season, `r params$player` averaged **`r season$threepg`** three pointers per game. In the past 5 games played, `r params$player` is averaging **`r season_last5$threepg` three pointers per game: `r paste0(round(((season_last5$threepg / season$threepg) - 1)* 100,0),"%")` change**
* `r params$player` has made at least 1 three pointer in **`r paste0(round(mean(dat_fg3m$fg3m >= 1),2)*100,"%")`** of all games played this season. The most 3 pointers `r params$player` has made in all games played this season is **`r max(dat_fg3m$fg3m)`**
* During the regular season, `r params$player` has averaged **`r round(pts_per_attempt_avg,2)` points per attempt** in `r pts_per_attempt_n` games played
