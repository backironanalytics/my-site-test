...
Points {data-navmenu="OU Analytics" data-navmenu-icon="fa-solid fa-filter"}
=======================================================================

Row
-----------------------------------------------------------------------

### Points Over Hit Rates {data-width=1400}

```{r points hit rate pt2, echo = FALSE, warning = FALSE, message = FALSE}

dat <- playerdata %>% filter(idPlayer == params$id) %>% 
  select(dateGame, idGame, slugOpponent, idPlayer, namePlayer, fgm,fga,pts,treb,ast,stl,blk,fg3m,fg3a,tov,
         plusminus,minutes,locationGame, countDaysRestPlayer, isB2B, isB2BFirst, isB2BSecond,typeSeason,slugSeason,ftm) %>% 
  arrange(desc(dateGame)) %>% mutate(tripdoub = ifelse(pts >= 10 & treb >= 10 & ast >= 10, "Y","N"),pt_reb_ast_total = pts+treb+ast)
...
hit_rate <- seq(from = season_avg-5,to = season_avg +5, by =1)
...
p <- df %>% mutate(Over = Over/100) %>% ggplot(aes(x = OU, y = Over, label = paste0(round(Over*100,0),"%"))) + geom_line(color = "grey", size = 2) + geom_point(shape = 21, color = "black", fill = primary_color, size = 6) + theme_minimal() + scale_x_continuous(breaks = seq(0.5,100.5,1)) + labs(x = "0/U", y = "Over Hit Rate", title = toupper(paste(player_name,"Over Hit Rates")), subtitle = "Current Regular Season") + geom_text(position = position_dodge(width = 0.9),vjust = -2) + scale_y_continuous(labels = scales::percent) + geom_hline(yintercept = .50, linetype = "dashed")+ 
  theme(plot.title = element_text(hjust = 0.5, vjust = -1, face = "bold"),
        plot.subtitle = element_text(hjust = 0.5, size = 12, vjust = -2, face = "bold")) + geom_vline(xintercept = season_avg, linetype = "dashed")
...
```{r points hit rate pts game log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}

seasontable <- player_data %>% left_join(play_play_rebounds %>% filter(numberPeriod == 1) %>% group_by(idGame) %>% summarize(firstqrebounds = n()), by = "idGame") %>% mutate(firstqrebounds = replace_na(firstqrebounds,0)) %>% left_join(play_play_assists %>% filter(numberPeriod == 1) %>% group_by(idGame) %>% summarize(firstqassists = n()), by = "idGame") %>% mutate(firstqassists = replace_na(firstqassists,0)) %>% left_join(play_play_makes %>% filter(numberPeriod == 1) %>% group_by(idGame,namePlayer1,slugScore,scoreHome,scoreAway,score_type) %>% summarize(n = n(),firstqpoints = sum(pts)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason) %>% summarize(n = n()), by = "idGame") %>% group_by(idGame) %>% summarize(firstqpoints = sum(firstqpoints)), by = "idGame") %>% mutate(firstqpoints = replace_na(firstqpoints,0)) %>% left_join(play_play_makes %>% group_by(idGame,namePlayer1,slugScore,scoreHome,scoreAway,score_type) %>% summarize(n = n()) %>% mutate(first = ifelse((scoreHome >0 & scoreAway == 0) | (scoreAway > 0 & scoreHome == 0),1,0))%>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason) %>% summarize(n = n()), by = "idGame") %>% group_by(idGame,first) %>% summarize(n = n()) %>% filter(first == 1), by = "idGame") %>% mutate(first = replace_na((first),0)) %>% filter(typeSeason == "Regular Season", slugSeason == current_season) %>% mutate(opponent = ifelse(locationGame == "A",paste0("@ ",slugOpponent),paste0("vs ",slugOpponent))) %>% group_by(dateGame,opponent,outcomeGame) %>% summarize(n = n(), total_pts = sum(pts), total_reb = sum(treb), total_ast = sum(ast), total_stl = sum(stl), total_blk = sum(blk), total_fgm = sum(fgm), total_fga = sum(fga), total_fg3m = sum(fg3m), total_minutes = sum(minutes), total_tov = sum(tov), total_first = sum(first), totalfqpts = sum(firstqpoints), totalfqast = sum(firstqassists), totalfqtreb = sum(firstqrebounds), totalftm = sum(ftm), total_fg3a = sum(fg3a))  %>% mutate(ppg = round(total_pts/n,1), rpg = round(total_reb/n,1), apg = round(total_ast/n,1), spg = round(total_stl/n,1), bpg = round(total_blk/n,1), fg_percentage = round((total_fgm/total_fga)*100,1), fg3_percentage = round((total_fg3m/total_fg3a)*100,1), min = round(total_minutes/n,1),threepg = round(total_fg3m/n,1),threeattempt = round(total_fg3a/n,1), tovpg = round(total_tov/n,1), pts_ast = round((total_pts+total_ast)/n,1), pts_reb_ast = round((total_reb+total_pts+total_ast)/n,1), pts_reb = round((total_pts+total_reb)/n,1), ast_reb = round((total_ast+total_reb)/n,1),firstqppg = round(totalfqpts/n,1), firstqapg = round(totalfqast/n,1), firstqrpg = round(totalfqtreb/n,1), pts_per_attempt = round(total_pts/total_fga,2), ftmpg = round(totalftm/n,1),stl_blk = round((total_blk+total_stl)/n,1), fgapg = round(total_fga/n,1),fgmpg = round(total_fgm/n,1) ) %>% select(dateGame,opponent,outcomeGame,n,min,ppg,rpg,apg,spg,bpg,threepg,threeattempt,fg_percentage,fgmpg,fgapg,fg3_percentage,ftmpg,tovpg, pts_reb_ast, pts_ast,pts_reb,ast_reb,stl_blk, total_first,firstqppg,firstqapg,firstqrpg, pts_per_attempt) %>%
  rename(GP = n,
        Opponent = opponent,
        Outcome = outcomeGame,
        Min = min,
         ...
         Blocks = bpg,
         Turnovers = tovpg,
         `Field Goals Made` = fgmpg,
         `Field Goals Attempted` = fgapg,
         `Three Pointers Made` = threepg,
         `Three Pointers Attempted` = threeattempt,
         `Free Throws Made` = ftmpg,
         `FG%` = fg_percentage,
         `3FG%` = fg3_percentage,
         `Pts+Reb+Ast` = pts_reb_ast,
         `Pts+Ast` = pts_ast,
         ...
         `Stl+Blk` = stl_blk,
         `First Scorer Count` = total_first,
         `First Quarter Points` = firstqppg,
         `First Quarter Assists` = firstqapg,
         `First Quarter Rebounds` = firstqrpg,
         `Pts Per Attempt` = pts_per_attempt,
        Date = dateGame) 


formattable(seasontable %>% select(Date,Opponent,Outcome,Points), align = "c", list(Points = color_tile("white","gold")))



```

Assists {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Assists Over Hit Rates {data-width=1400}

```{r points hit rate assists2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate ast game log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,Assists), align = "c", list(Assists = color_tile("white","gold")))
...
Rebounds {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Rebounds Over Hit Rates {data-width=1400}

```{r points hit rate rebounds2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate reb game log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,Rebounds), align = "c", list(Rebounds = color_tile("white","gold")))
...
Steals {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Steals Over Hit Rates {data-width=1400}

```{r points hit rate steals2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate stl game log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,Steals), align = "c", list(Steals = color_tile("white","gold")))



```

Three Pointers Made {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Three Pointers Made Over Hit Rates {data-width=1400}

```{r points hit rate fg3m2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate fg3mgame log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,`Three Pointers Made`), align = "c", list(`Three Pointers Made` = color_tile("white","gold")))
...
Three Pointers Attempted {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Three Pointers Attempted Over Hit Rates {data-width=1400}

```{r points hit rate fg3a2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate fg3agame log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,`Three Pointers Attempted`), align = "c", list(`Three Pointers Attempted` = color_tile("white","gold")))
...
Turnovers {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Turnovers Over Hit Rates {data-width=1400}

```{r points hit rate tov2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate tov log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,Turnovers), align = "c", list(Turnovers = color_tile("white","gold")))



```

Blocks {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Blocks Over Hit Rates {data-width=1400}

```{r points hit rate blk2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate blk log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,Blocks), align = "c", list(Blocks = color_tile("white","gold")))
...
Total Free Throws Made {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Free Throws Made Over Hit Rates {data-width=1400}

```{r points hit rate ftm2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate ftm log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,`Free Throws Made`), align = "c", list(`Free Throws Made` = color_tile("white","gold")))



```

Total Field Goals Made {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Field Goals Made Over Hit Rates {data-width=1400}

```{r points hit rate fgm, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate fgm log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,`Field Goals Made`), align = "c", list(`Field Goals Made` = color_tile("white","gold")))
...
Total Field Goals Attempted {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Field Goals Attempted Over Hit Rates {data-width=1400}

```{r points hit rate fga, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate fga log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,`Field Goals Attempted`), align = "c", list(`Field Goals Attempted` = color_tile("white","gold")))
...
Points + Rebounds + Assists {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Points + Rebounds + Assists Over Hit Rates {data-width=1400}

```{r points hit rate pt_reb_ast2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate pt_reb_ast game log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,`Pts+Reb+Ast`), align = "c", list(`Pts+Reb+Ast` = color_tile("white","gold")))



```

Points + Assists {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Points + Assists Over Hit Rates {data-width=1400}

```{r points hit rate pt_ast2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate pt_ast game log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,`Pts+Ast`), align = "c", list(`Pts+Reb` = color_tile("white","gold")))
...
Points + Rebounds {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Points + Rebounds Over Hit Rates {data-width=1400}

```{r points hit rate pt_reb2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate pt_reb game log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,`Pts+Reb`), align = "c", list(`Pts+Reb` = color_tile("white","gold")))
...
Rebounds + Assists {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Rebounds + Assists Over Hit Rates {data-width=1400}

```{r points hit rate reb_ast2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate reb_ast game log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,`Reb+Ast`), align = "c", list(`Reb+Ast` = color_tile("white","gold")))
...
Steals + Blocks {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### Steals + Blocks Over Hit Rates {data-width=1400}

```{r points hit rate stl_blk2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate stl_blk game log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,`Stl+Blk`), align = "c", list(`Stl+Blk` = color_tile("white","gold")))



```

First Quarter Points {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### First Quarter Points Over Hit Rates {data-width=1400}

```{r points hit rate fqp2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate fqp game log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,`First Quarter Points`), align = "c", list(`First Quarter Points` = color_tile("white","gold")))
...
First Quarter Assists {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### First Quarter Assists Over Hit Rates {data-width=1400}

```{r points hit rate fqa2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate fqa game log, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center"}



formattable(seasontable %>% select(Date,Opponent,Outcome,`First Quarter Assists`), align = "c", list(`First Quarter Assists` = color_tile("white","gold")))



```

First Quarter Rebounds {data-navmenu="OU Analytics"}
=======================================================================

Row
-----------------------------------------------------------------------

### First Quarter Rebounds Over Hit Rates {data-width=1400}

```{r points hit rate fqr2, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r points hit rate fqr game log, echo = FALSE, warning = FALSE, message = FALSE}



formattable(seasontable %>% select(Date,Opponent,Outcome,`First Quarter Rebounds`), align = "c", list(`First Quarter Rebounds` = color_tile("white","gold")))
...
Points {data-navmenu="Leading Indicator Analysis" data-navmenu-icon="fa-chart-area"}
=======================================================================

Row
-----------------------------------------------------------------------

### First Quarter Leading Indicator - Points {data-width=1400}
...
p2 <- df %>% ggplot(aes(x = Points, y = Probability)) + geom_line(size = 2, color = "#69b3a2") + geom_point(size = 2, color = "white") + geom_area(fill = "#69b3a2", alpha = 0.4) + theme_minimal() + labs(y = "Probability %") + geom_hline(yintercept = 50, color = "red3", linetype = "dashed") + scale_x_continuous(breaks = seq(0,90,1))

ggplotly(p2) %>% layout(title = list(text = toupper(paste(player_name,"Leading Indicator Analysis",'<br>','<sup>',paste("Likelihood that this total game point total is scored when ",player_name," scores at least\n ",round(first,1)," points in 1Q: Current Regular Season Only")))))
...
```{r Quarterly points lead2, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", error = TRUE}




formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this total game point total is scored when ",player_name," scores at least ",round(first,1)," points in 1Q: Current Regular Season Only"))
...
Assists {data-navmenu="Leading Indicator Analysis"}
=======================================================================
...
### First Quarter Leading Indicator - Assists {data-width=1400}
...
p2 <- df %>% ggplot(aes(x = Assists, y = Probability)) + geom_line(size = 2, color = "#69b3a2") + geom_point(size = 2, color = "white") + geom_area(fill = "#69b3a2", alpha = 0.4) + theme_minimal() + labs(y = "Probability %") + geom_hline(yintercept = 50, color = "red3", linetype = "dashed")

ggplotly(p2) %>% layout(title = list(text = toupper(paste(player_name,"Leading Indicator Analysis",'<br>','<sup>',paste("Likelihood that this game assist total happens when ",player_name," has at least\n ",round(first,1)," assists in 1Q: Current Regular Season Only")))))
...
```{r Quarterly assists lead2, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", error = TRUE}



formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this game assist total happens when ",player_name," has at least ",round(first,1)," assists in 1Q: Current Regular Season Only"))



```

Rebounds {data-navmenu="Leading Indicator Analysis"}
=======================================================================
...
### First Quarter Leading Indicator - Rebounds {data-width=1400}
...
p2 <- df %>% ggplot(aes(x = Rebounds, y = Probability)) + geom_line(size = 2, color = "#69b3a2") + geom_point(size = 2, color = "white") + geom_area(fill = "#69b3a2", alpha = 0.4) + theme_minimal() + labs(y = "Probability %") + geom_hline(yintercept = 50, color = "red3", linetype = "dashed")

ggplotly(p2) %>% layout(title = list(text = toupper(paste(player_name,"Leading Indicator Analysis",'<br>','<sup>',paste("Likelihood that this game rebound total happens when ",player_name," has at least\n ",round(first,1)," rebounds in 1Q: Current Regular Season Only")))))
...
```{r Quarterly rebounds lead2, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", error = TRUE}




formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this game rebound total happens when ",player_name," has at least ",round(first,1)," rebounds in 1Q: Current Regular Season Only"))
...
Steals {data-navmenu="Leading Indicator Analysis"}
=======================================================================
...
### First Quarter Leading Indicator - Steals {data-width=1400}
...
p2 <- df %>% ggplot(aes(x = Steals, y = Probability)) + geom_line(size = 2, color = "#69b3a2") + geom_point(size = 2, color = "white") + geom_area(fill = "#69b3a2", alpha = 0.4) + theme_minimal() + labs(y = "Probability %") + geom_hline(yintercept = 50, color = "red3", linetype = "dashed")

ggplotly(p2) %>% layout(title = list(text = toupper(paste(player_name,"Leading Indicator Analysis",'<br>','<sup>',paste("Likelihood that this game steal total happens when ",player_name," has at least\n ",round(first,1)," steals in 1Q: Current Regular Season Only")))))
...
```{r Quarterly steals lead2, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", error = TRUE}

 

formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this game steal total happens when ",player_name," has at least ",round(first,1)," steals in 1Q: Current Regular Season Only"))


```

Blocks {data-navmenu="Leading Indicator Analysis"}
=======================================================================
...
### First Quarter Leading Indicator - Blocks {data-width=1400}
...
p2 <- df %>% ggplot(aes(x = Blocks, y = Probability)) + geom_line(size = 2, color = "#69b3a2") + geom_point(size = 2, color = "white") + geom_area(fill = "#69b3a2", alpha = 0.4) + theme_minimal() + labs(y = "Probability %") + geom_hline(yintercept = 50, color = "red3", linetype = "dashed")

ggplotly(p2) %>% layout(title = list(text = toupper(paste(player_name,"Leading Indicator Analysis",'<br>','<sup>',paste("Likelihood that this game block total happens when ",player_name," has at least\n ",round(first,1)," blocks in 1Q: Current Regular Season Only")))))
...
```{r Quarterly blocks lead2, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", error = TRUE}

 

formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this game block total happens when ",player_name," has at least ",round(first,1)," blocks in 1Q: Current Regular Season Only"))


```

Three Pointers {data-navmenu="Leading Indicator Analysis"}
=======================================================================
...
### First Quarter Leading Indicator - Three Pointers {data-width=1400}
...
p2 <- df %>% ggplot(aes(x = Threes, y = Probability)) + geom_line(size = 2, color = "#69b3a2") + geom_point(size = 2, color = "white") + geom_area(fill = "#69b3a2", alpha = 0.4) + theme_minimal() + labs(y = "Probability %") + geom_hline(yintercept = 50, color = "red3", linetype = "dashed")

ggplotly(p2) %>% layout(title = list(text = toupper(paste(player_name,"Leading Indicator Analysis",'<br>','<sup>',paste("Likelihood that this threes made total is scored when ",player_name," makes at least\n ",round(first,1)," threes in 1Q: Current Regular Season Only")))))

```

### First Quarter Leading Indicator - Three Pointers

```{r Quarterly three points lead2, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", error = TRUE}


formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this threes made total is scored when ",player_name," makes at least ",round(first,1)," threes in 1Q: Current Regular Season Only"))
...
Pts+Reb+Ast {data-navmenu="Leading Indicator Analysis"}
=======================================================================

Row
-----------------------------------------------------------------------

### First Quarter Leading Indicator - Pts+Reb+Ast {data-width=1400}
...
p2 <- df %>% ggplot(aes(x = `Pts.Reb.Ast`, y = Probability)) + geom_line(size = 2, color = "#69b3a2") + geom_point(size = 2, color = "white") + geom_area(fill = "#69b3a2", alpha = 0.4) + theme_minimal() + labs(y = "Probability %") + geom_hline(yintercept = 50, color = "red3", linetype = "dashed")

ggplotly(p2) %>% layout(title = list(text = toupper(paste(player_name,"Leading Indicator Analysis",'<br>','<sup>',paste("Likelihood that this Pts+Reb+Ast total is attained when ",player_name," has at least\n ",round(first,1)," Pts+Reb+Ast in 1Q: Current Regular Season Only")))))

```

### First Quarter Leading Indicator - Pts+Reb+Ast

```{r Quarterly ptsrebast points lead2, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", error = TRUE}



formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this Pts+Reb+Ast total is attained when ",player_name," has at least ",round(first,1)," Pts+Reb+Ast in 1Q: Current Regular Season Only"))

```


Pts+Ast {data-navmenu="Leading Indicator Analysis"}
=======================================================================
...
### First Quarter Leading Indicator - Pts+Ast {data-width=1400}
...
p2 <- df %>% ggplot(aes(x = `Pts.Ast`, y = Probability)) + geom_line(size = 2, color = "#69b3a2") + geom_point(size = 2, color = "white") + geom_area(fill = "#69b3a2", alpha = 0.4) + theme_minimal() + labs(y = "Probability %") + geom_hline(yintercept = 50, color = "red3", linetype = "dashed")

ggplotly(p2) %>% layout(title = list(text = toupper(paste(player_name,"Leading Indicator Analysis",'<br>','<sup>',paste("Likelihood that this Pts+Ast total is attained when ",player_name," has at least\n ",round(first,1)," Pts+Ast in 1Q: Current Regular Season Only")))))

```

### First Quarter Leading Indicator - Pts+Ast

```{r Quarterly ptsast lead2, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", error = TRUE}



formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this Pts+Ast total is attained when ",player_name," has at least ",round(first,1)," Pts+Ast in 1Q: Current Regular Season Only"))

```


Pts+Reb {data-navmenu="Leading Indicator Analysis"}
=======================================================================
...
### First Quarter Leading Indicator - Pts+Reb {data-width=1400}
...
p2 <- df %>% ggplot(aes(x = `Pts.Reb`, y = Probability)) + geom_line(size = 2, color = "#69b3a2") + geom_point(size = 2, color = "white") + geom_area(fill = "#69b3a2", alpha = 0.4) + theme_minimal() + labs(y = "Probability %") + geom_hline(yintercept = 50, color = "red3", linetype = "dashed")

ggplotly(p2) %>% layout(title = list(text = toupper(paste(player_name,"Leading Indicator Analysis",'<br>','<sup>',paste("Likelihood that this Pts+Reb total is attained when ",player_name," has at least\n ",round(first,1)," Pts+Reb in 1Q: Current Regular Season Only")))))

```


### First Quarter Leading Indicator - Pts+Reb
```{r Quarterly ptsreb lead2, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", error = TRUE}


formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this Pts+Reb total is attained when ",player_name," has at least ",round(first,1)," Pts+Reb in 1Q: Current Regular Season Only"))

```



Reb+Ast {data-navmenu="Leading Indicator Analysis"}
=======================================================================
...
### First Quarter Leading Indicator - Reb+Ast {data-width=1400}
...
p2 <- df %>% ggplot(aes(x = `Reb.Ast`, y = Probability)) + geom_line(size = 2, color = "#69b3a2") + geom_point(size = 2, color = "white") + geom_area(fill = "#69b3a2", alpha = 0.4) + theme_minimal() + labs(y = "Probability %") + geom_hline(yintercept = 50, color = "red3", linetype = "dashed")

ggplotly(p2) %>% layout(title = list(text = toupper(paste(player_name,"Leading Indicator Analysis",'<br>','<sup>',paste("Likelihood that this Reb+Ast total is attained when ",player_name," has at least\n ",round(first,1)," Reb+Ast in 1Q: Current Regular Season Only")))))

```

### First Quarter Leading Indicator - Reb+Ast 

```{r Quarterly astreb lead2, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", error = TRUE}


formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this Reb+Ast total is attained when ",player_name," has at least ",round(first,1)," Reb+Ast in 1Q: Current Regular Season Only"))

```


Stl+Blk {data-navmenu="Leading Indicator Analysis"}
=======================================================================

Row
-----------------------------------------------------------------------

### First Quarter Leading Indicator - Stl+Blk {data-width=1400}

```{r Quarterly stlblk lead, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", error = TRUE}
...
p2 <- df %>% ggplot(aes(x = `Stl.Blk`, y = Probability)) + geom_line(size = 2, color = "#69b3a2") + geom_point(size = 2, color = "white") + geom_area(fill = "#69b3a2", alpha = 0.4) + theme_minimal() + labs(y = "Probability %") + geom_hline(yintercept = 50, color = "red3", linetype = "dashed")

ggplotly(p2) %>% layout(title = list(text = toupper(paste(player_name,"Leading Indicator Analysis",'<br>','<sup>',paste("Likelihood that this Stl+Blk total is attained when ",player_name," has at least\n ",round(first,1)," Stl+Blk in 1Q: Current Regular Season Only")))))

```

### First Quarter Leading Indicator - Stl+Blk 

```{r Quarterly stlblk lead2, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.align = "center", error = TRUE}



formattable(df, align = c("c","c"), list(Probability = color_bar("orange")), caption = paste("Likelihood that this Stl+Blk total is attained when ",player_name," has at least ",round(first,1)," Stl+Blk in 1Q: Current Regular Season Only"))
...
<center> <p>Prop Report For `r player_name` Created on `r Sys.Date()`</p></center>
...
#profile <- invisible(capture.output(player_profiles(params$ids = params$id[1], nest_data = F, return_message = F)))
