...
Type <- c("2024/25 Regular Season","Last 10 GP","Last 5 GP","2024/25 Regular Season Home","2024/25 Regular Season Away",paste("Against",next_opponent,"Regular Season"),"Second Night of Back of Back")
combined <- bind_rows(season_frame,season_last10_frame,season_last5_frame,season_home_frame,season_away_frame,season_opponent_frame,b2b_frame)
...
avg_lastx <- player_data %>% filter(slugSeason == current_season) %>% arrange(desc(dateGame)) %>% head(params$games) %>% group_by(dateGame) %>% summarize(n = n()) %>% left_join(play_play_makes %>% filter(numberPeriod == 1) %>% group_by(dateGame) %>% summarize(pts = sum(pts)), by = "dateGame") %>% mutate(pts = replace_na(pts,0)) %>% summarize(avg = mean(pts)) %>% pull(avg)
...
p <- firstq_makes %>% filter(slugSeason == current_season)%>% ggplot(aes(dateGame,pts, label = slugOpponent)) + geom_line() + geom_point(aes(color = locationGame, shape = typeSeason))+ theme_minimal() +labs(x = "Date") + scale_y_continuous(breaks = seq(0,90,1)) + ggtitle(toupper(paste(params$player,"1st Quarter Pts")), subtitle = toupper(paste("Season Avg:",round(avg,1),"/","Avg Last",params$games,"games:",round(avg_lastx,1)))) + geom_hline(yintercept = avg) + geom_smooth(span = 0.6) + scale_color_manual(values = c(primary_color,secondary_color))+ scale_shape_manual(values = c(15,16)) + scale_x_date(breaks = "1 month", date_labels = "%b")
...
avg_lastx <- mean(firstq_assists %>% filter(slugSeason == current_season) %>% arrange(desc(dateGame)) %>% head(params$games) %>% pull(assists))

p <- firstq_assists %>% filter(slugSeason == current_season) %>% ggplot(aes(dateGame,assists, label = slugOpponent)) + geom_line() + geom_point(aes(color = locationGame, shape = typeSeason))+ theme_minimal() +labs(x = "Date") + scale_y_continuous(breaks = seq(0,90,1)) + ggtitle(toupper(paste(params$player,"1st Quarter Assists")), subtitle = toupper(paste("Season Avg:", round(avg,1),"/","Avg Last",params$games,"games:",round(avg_lastx,1)))) + geom_smooth(span = 0.6) + geom_hline(yintercept = avg) + scale_color_manual(values = c(primary_color,secondary_color))+ scale_shape_manual(values = c(15,16)) + scale_x_date(breaks = "1 month", date_labels = "%b")
...
avg_lastx <- mean(firstq_rebounds %>% filter(slugSeason == current_season) %>% arrange(desc(dateGame)) %>% head(params$games) %>% pull(rebounds))

p <- firstq_rebounds %>% filter(slugSeason == current_season) %>% ggplot(aes(dateGame,rebounds, label = slugOpponent)) + geom_line() + geom_point(aes(color = locationGame, shape = typeSeason))+ theme_minimal() +labs(x = "Date") + scale_y_continuous(breaks = seq(0,90,1)) + ggtitle(toupper(paste(params$player,"1st Quarter Rebounds")), subtitle = toupper(paste("Season Avg:", round(avg,1),"/","Avg Last",params$games,"games:",round(avg_lastx,1)))) + geom_smooth(span = 0.6) + geom_hline(yintercept = avg) + scale_color_manual(values = c(primary_color,secondary_color))+ scale_shape_manual(values = c(15,16)) + scale_x_date(breaks = "1 month", date_labels = "%b")
...
# Grab team shot data
team <- teams_shots(team_ids = team_id, seasons = 2025, season_types = "Regular Season", return_message = FALSE)
...
names <- colnames(player)
error <- as.data.frame(matrix(c(player_name[1],as.integer(seq(1:ncol(player))*0)),nrow = 1, ncol = ncol(player)))
error <- error %>% mutate(V28 = ifelse(V28 == 0,1000,0),V29 = ifelse(V29 == 0,1000,0))
colnames(error) <- names
error[,-1] <- sapply(error[,-1],as.numeric)
player_q1_frame <- if(nrow(player_q1) == 0){error}else{player_q1}
player_q1_frame <- player_q1_frame %>% mutate(isShotAttempted = ifelse(isShotAttempted == 0, FALSE, TRUE), isShotMade = ifelse(isShotMade == 0, FALSE, TRUE))

shot_summary_q1 <- shot_summary %>% filter(numberPeriod == 1)

shot_summary_2 <- as.data.frame(shot_summary_q1 %>% group_by(zoneBasic) %>% summarize(makes = sum(makes),attemps = sum(attempts)) %>% mutate(fg = makes/attemps) %>%select(zoneBasic,fg) %>% pivot_wider(names_from = zoneBasic, values_from = fg)) %>% replace(is.na(.),0)
...
p1 <- plot_court(court_themes$custom2, use_short_three = F) +
  geom_point(data = player_q1_frame, aes(x = x, y = y, color = player_q1_frame$isShotMade, fill = player_q1_frame$isShotMade), 
             size =2, shape = 21, stroke = .5) +  
  scale_color_manual(values = c("green4","red3"), aesthetics = "color", breaks=c("TRUE", "FALSE"), labels=c("Made", "Missed")) +
  scale_fill_manual(values = c("green4","red3"), aesthetics = "fill", breaks=c("TRUE", "FALSE"), labels=c("Made", "Missed")) +
  scale_x_continuous(limits = c(-27.5, 27.5)) +
...
colnames(error) <- names
error[,-1] <- sapply(error[,-1],as.numeric)
player_q2_frame <- if(nrow(player_q2) == 0){error}else{player_q2}
player_q2_frame <- player_q2_frame %>% mutate(isShotAttempted = ifelse(isShotAttempted == 0, FALSE, TRUE), isShotMade = ifelse(isShotMade == 0, FALSE, TRUE))

shot_summary_q2 <- shot_summary %>% filter(numberPeriod == 2)

shot_summary_2 <- as.data.frame(shot_summary_q2 %>% group_by(zoneBasic) %>% summarize(makes = sum(makes),attemps = sum(attempts)) %>% mutate(fg = makes/attemps) %>%select(zoneBasic,fg) %>% pivot_wider(names_from = zoneBasic, values_from = fg))%>% replace(is.na(.),0)
...
p1 <- plot_court(court_themes$custom2, use_short_three = F) +
  geom_point(data = player_q2_frame, aes(x = x, y = y, color = player_q2_frame$isShotMade, fill = player_q2_frame$isShotMade), 
             size =2, shape = 21, stroke = .5) +  
  scale_color_manual(values = c("green4","red3"), aesthetics = "color", breaks=c("TRUE", "FALSE"), labels=c("Made", "Missed")) +
  scale_fill_manual(values = c("green4","red3"), aesthetics = "fill", breaks=c("TRUE", "FALSE"), labels=c("Made", "Missed")) +
  scale_x_continuous(limits = c(-27.5, 27.5)) +
...
colnames(error) <- names
error[,-1] <- sapply(error[,-1],as.numeric)
player_q3_frame <- if(nrow(player_q3) == 0){error}else{player_q3}
player_q3_frame <- player_q3_frame %>% mutate(isShotAttempted = ifelse(isShotAttempted == 0, FALSE, TRUE), isShotMade = ifelse(isShotMade == 0, FALSE, TRUE))

shot_summary_q3 <- shot_summary %>% filter(numberPeriod == 3)

shot_summary_2 <- as.data.frame(shot_summary_q3 %>% group_by(zoneBasic) %>% summarize(makes = sum(makes),attemps = sum(attempts)) %>% mutate(fg = makes/attemps) %>%select(zoneBasic,fg) %>% pivot_wider(names_from = zoneBasic, values_from = fg))%>% replace(is.na(.),0)
...
p1 <- plot_court(court_themes$custom2, use_short_three = F) +
  geom_point(data = player_q3_frame, aes(x = x, y = y, color = player_q3_frame$isShotMade, fill = player_q3_frame$isShotMade), 
             size =2, shape = 21, stroke = .5) +  
  scale_color_manual(values = c("green4","red3"), aesthetics = "color", breaks=c("TRUE", "FALSE"), labels=c("Made", "Missed")) +
  scale_fill_manual(values = c("green4","red3"), aesthetics = "fill", breaks=c("TRUE", "FALSE"), labels=c("Made", "Missed")) +
  scale_x_continuous(limits = c(-27.5, 27.5)) +
...
colnames(error) <- names
error[,-1] <- sapply(error[,-1],as.numeric)
player_q4_frame <- if(nrow(player_q4) == 0){error}else{player_q4}
player_q4_frame <- player_q4_frame %>% mutate(isShotAttempted = ifelse(isShotAttempted == 0, FALSE, TRUE), isShotMade = ifelse(isShotMade == 0, FALSE, TRUE))

shot_summary_q4 <- shot_summary %>% filter(numberPeriod == 4)

shot_summary_2 <- as.data.frame(shot_summary_q4 %>% group_by(zoneBasic) %>% summarize(makes = sum(makes),attemps = sum(attempts)) %>% mutate(fg = makes/attemps) %>%select(zoneBasic,fg) %>% pivot_wider(names_from = zoneBasic, values_from = fg)) %>% replace(is.na(.),0)
...
p1 <- plot_court(court_themes$custom2, use_short_three = F) +
  geom_point(data = player_q4_frame, aes(x = x, y = y, color = player_q4_frame$isShotMade, fill = player_q4_frame$isShotMade), 
             size =2, shape = 21, stroke = .5) +  
  scale_color_manual(values = c("green4","red3"), aesthetics = "color", breaks=c("TRUE", "FALSE"), labels=c("Made", "Missed")) +
  scale_fill_manual(values = c("green4","red3"), aesthetics = "fill", breaks=c("TRUE", "FALSE"), labels=c("Made", "Missed")) +
  scale_x_continuous(limits = c(-27.5, 27.5)) +
...
```{r  season shot chart facet test, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', error = TRUE}
...
season_opponent_table <- player_data %>% left_join(play_play_rebounds %>% filter(numberPeriod == 1) %>% group_by(idGame) %>% summarize(firstqrebounds = n()), by = "idGame") %>% mutate(firstqrebounds = replace_na(firstqrebounds,0)) %>% left_join(play_play_assists %>% filter(numberPeriod == 1) %>% group_by(idGame) %>% summarize(firstqassists = n()), by = "idGame") %>% mutate(firstqassists = replace_na(firstqassists,0)) %>% left_join(play_play_makes %>% filter(numberPeriod == 1) %>% group_by(idGame,namePlayer1,slugScore,scoreHome,scoreAway,score_type) %>% summarize(n = n(),firstqpoints = sum(pts)) %>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason) %>% summarize(n = n()), by = "idGame") %>% group_by(idGame) %>% summarize(firstqpoints = sum(firstqpoints)), by = "idGame") %>% mutate(firstqpoints = replace_na(firstqpoints,0)) %>% left_join(play_play_makes %>% group_by(idGame,namePlayer1,slugScore,scoreHome,scoreAway,score_type) %>% summarize(n = n()) %>% mutate(first = ifelse((scoreHome >0 & scoreAway == 0) | (scoreAway > 0 & scoreHome == 0),1,0))%>% left_join(gamedata %>% group_by(idGame,dateGame,typeSeason) %>% summarize(n = n()), by = "idGame") %>% group_by(idGame,first) %>% summarize(n = n()) %>% filter(first == 1), by = "idGame") %>% mutate(first = replace_na((first),0)) %>% filter(typeSeason == "Regular Season", locationGame == "H", slugSeason == current_season) %>% group_by(slugOpponent) %>% summarize(n = n(), total_pts = sum(pts), total_reb = sum(treb), total_ast = sum(ast), total_stl = sum(stl), total_blk = sum(blk), total_fgm = sum(fgm), total_fga = sum(fga), total_fg3m = sum(fg3m), total_fg3a = sum(fg3a), total_minutes = sum(minutes), total_tov = sum(tov), total_first = sum(first), totalfqpts = sum(firstqpoints), totalfqast = sum(firstqassists), totalfqtreb = sum(firstqrebounds))  %>% mutate(ppg = round(total_pts/n,1), rpg = round(total_reb/n,1), apg = round(total_ast/n,1), spg = round(total_stl/n,1), bpg = round(total_blk/n,1), fg_percentage = round((total_fgm/total_fga)*100,1), fg3_percentage = round((total_fg3m/total_fg3a)*100,1), min = round(total_minutes/n,1),threepg = round(total_fg3m/n,1), tovpg = round(total_tov/n,1), pts_reb_ast = round((total_reb+total_pts+total_ast)/n,1), pts_ast = round((total_pts+total_ast)/n,1), pts_reb = round((total_pts+total_reb)/n,1), ast_reb = round((total_ast+total_reb)/n,1),firstqppg = round(totalfqpts/n,1), firstqapg = round(totalfqast/n,1), firstqrpg = round(totalfqtreb/n,1)) %>% select(slugOpponent,n,min,ppg,rpg,apg,spg,bpg,threepg,fg_percentage,fg3_percentage,tovpg,pts_reb_ast,pts_ast,pts_reb,ast_reb, total_first,firstqppg,firstqapg,firstqrpg) %>%
  rename(GP = n,
        Opponent = slugOpponent, 
        Min = min,
         PPG = ppg,
